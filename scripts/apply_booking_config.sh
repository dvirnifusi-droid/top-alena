#!/usr/bin/env bash
# Additive, idempotent: add ReservationSettings.booking_config to EVERY schema.
# Run BEFORE rebuilding api (the new Prisma client selects the column).
set -uo pipefail
cd /opt/top-alena

SQL='ALTER TABLE "ReservationSettings" ADD COLUMN IF NOT EXISTS "booking_config" JSONB;'
