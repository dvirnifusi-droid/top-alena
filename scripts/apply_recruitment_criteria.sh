#!/usr/bin/env bash
# Additive, idempotent: add RestaurantProfile.recruitment_criteria to EVERY schema.
# Run BEFORE rebuilding api (the new Prisma client selects the column).
set -uo pipefail
cd /opt/top-alena

SQL='ALTER TABLE "RestaurantProfile" ADD COLUMN IF NOT EXISTS "recruitment_criteria" JSONB;'
