...
export const googleSyncRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/connect', async (req, reply) => {
    ...
    if (!phone) return reply.code(400).type('text/html').send('<h2>missing ?phone= param</h2>');
  ...
  app.get('/status', async (req, reply) => {
    const phone = String((req.query as any)?.phone || '');
    if (!phone) return reply.code(400).send({ error: 'missing_phone' });
