...
async function ensureThanksSettings(): Promise<void> {
...
async function ensureLeadToken(): Promise<void> {
  ...
  await dbx().$executeRawUnsafe(
    `ALTER TABLE "EventLead" ADD COLUMN IF NOT EXISTS "public_token" TEXT`).catch(() => {});
  await dbx().$executeRawUnsafe(
    `CREATE UNIQUE INDEX IF NOT EXISTS "EventLead_public_token_key"
  ...
  await dbx().$executeRawUnsafe(
    `ALTER TABLE "EventLead" ADD COLUMN IF NOT EXISTS "viewed_at" TIMESTAMPTZ`).catch(() => {});
  await dbx().$executeRawUnsafe(
    `ALTER TABLE "EventLead" ADD COLUMN IF NOT EXISTS "view_count" INTEGER DEFAULT 0`).catch(() => {});
}
...
const CUSTOMER_STAGE: Record<string, number> = {
 ...
 */
export async function getOrCreateLeadToken(leadId: string): Promise<string | null> {
  if (!leadId) return null;
  await ensureLeadToken();
  try {
    const rows: any[] = await dbx().$queryRawUnsafe(
      `SELECT public_token FROM "EventLead" WHERE id = $1`, leadId);
    const existing = rows?.[0]?.public_token;
    if (existing) return String(existing);
    const token = randomBytes(24).toString('base64url');
    await dbx().$executeRawUnsafe(
      `UPDATE "EventLead" SET public_token = $2 WHERE id = $1 AND public_token IS NULL`,
      leadId, token);
    const after: any[] = await dbx().$queryRawUnsafe(
      `SELECT public_token FROM "EventLead" WHERE id = $1`, leadId);
...
function readMeta(notes: any): any {
  try {
...
registerFn('getEventLeadByToken', async ({ body }: any) => {
  await ensureLeadToken();
  const token = String((body || {}).token || '').trim();
  // A short token means a truncated or hand-typed link, never a real one.
  if (token.length < 20) throw new Error('not_found');

  const rows: any[] = await dbx().$queryRawUnsafe(
    `SELECT id, contact_name, contact_phone, event_date, event_type, guest_count,
...
async function ensurePixelTable(): Promise<void> {
  await dbx().$executeRawUnsafe(`
...
registerFn('getMarketingPixels', async () => {
  ...
  const rows: any[] = await dbx().$queryRawUnsafe(
    `SELECT meta_pixel_id, meta_event_name, google_ads_id, google_ads_label, ga4_id, enabled
...
registerFn('setMarketingPixels', async ({ user, body }: any) => {
  ...
  await dbx().$executeRawUnsafe(
    `INSERT INTO "MarketingPixelSetting"
       (id, meta_pixel_id, meta_event_name, google_ads_id, google_ads_label, ga4_id, enabled)
     VALUES ($1,$2,$3,$4,$5,$6,$7)`,
    `px_${Date.now().toString(36)}`,
    clean(b.meta_pixel_id) || null,
    clean(b.meta_event_name, 40) || 'Lead',
    clean(b.google_ads_id) || null,
    clean(b.google_ads_label) || null,
    clean(b.ga4_id) || null,
    b.enabled !== false);
  return { ok: true };
});
...
registerFn('getEventThanksSettings', async () => {
  ...
  const rows: any[] = await dbx().$queryRawUnsafe(
    `SELECT video_url, video_title, response_text
...
registerFn('setEventThanksSettings', async ({ user, body }: any) => {
  if (!user?.id) throw new Error('unauthorized');
  if (!isAdminRole((user as any)?.role)) throw new Error('admin only');
  await ensureThanksSettings();
  const b = (body || {}) as any;
  ...
  await dbx().$executeRawUnsafe(
    `INSERT INTO "EventThanksSetting" (id, video_url, video_title, response_text)
     VALUES ($1,$2,$3,$4)`,
    `ets_${Date.now().toString(36)}`,
    safeUrl || null,
    String(b.video_title || '').slice(0, 120) || null,
    String(b.response_text || '').slice(0, 200) || null);
