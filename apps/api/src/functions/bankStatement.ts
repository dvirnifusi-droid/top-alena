...
async function ensureBulkPaidCol(): Promise<void> {
  await dbx().$executeRawUnsafe(
    `ALTER TABLE "Invoice" ADD COLUMN IF NOT EXISTS "bulk_paid_batch" TEXT`).catch(() => {});
  ...
  await dbx().$executeRawUnsafe(
    `ALTER TABLE "Invoice" ADD COLUMN IF NOT EXISTS "bulk_paid_prev" TEXT`).catch(() => {});
}

registerFn('markInvoicesPaidBefore', async ({ user, body }: any) => {
  await guard(user);
  ...
  const preview: any[] = await dbx().$queryRawUnsafe(
    `SELECT COUNT(*)::int AS count, COALESCE(SUM(ABS(i.total_amount)),0)::float AS total ${where}`,
    date).catch(() => []);
  const count = Number(preview[0]?.count) || 0;
  const total = Math.round(Number(preview[0]?.total) || 0);

  if (!apply) return { ok: true, dry_run: true, date, count, total };
  if (!count) return { ok: true, dry_run: false, date, count: 0, total: 0, batch: null };
...
registerFn('undoBulkPaid', async ({ user, body }: any) => {
  await guard(user);
  await ensureBulkPaidCol();
  const batch = String((body || {}).batch || '');
  if (!batch) throw new Error('batch_required');
  const r: any[] = await dbx().$queryRawUnsafe(
    `SELECT COUNT(*)::int c FROM "Invoice" WHERE bulk_paid_batch = $1`, batch).catch(() => []);
  await dbx().$executeRawUnsafe(
    `UPDATE "Invoice"
...
registerFn('listBulkPaidBatches', async ({ user }: any) => {
  await guard(user);
  await ensureBulkPaidCol();
  const rows: any[] = await dbx().$queryRawUnsafe(
    `SELECT bulk_paid_batch AS batch, COUNT(*)::int AS count,
