...
registerFn('publishCommissaryCatalog', async ({ user }) => {
  ...
  const active = catalog.filter((c: any) => c.active);
  const keys: string[] = [];
...
registerFn('getChainCommissary', async ({ user, body }) => {
  ...
  for (const l of lineRows) {
    ...
    bx.total_ils += rev; bx.cost_ils += cost; bx.lines += 1; bx._items.add(l.item_key);
  }
  ...
  const prodStatus: any[] = await (prisma as any).$queryRawUnsafe(
    `SELECT item_key, done, done_by, done_at FROM public."CommissaryProduction" WHERE chain_id=$1 AND order_date=$2`, chainId, date,
  ).catch(() => []);
  const statusByKey = new Map(prodStatus.map((s) => [s.item_key, s]));
  const production = [...items.values()].map((it) => {
    const st: any = statusByKey.get(it.item_key);
    const cat: any = costByKey.get(it.item_key);
...
registerFn('markCommissaryProduction', async ({ user, body }) => {
  ...
  if (!chainId || !itemKey || !/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error('chain_id + order_date + item_key required');
  await assertChainOwner(user, chainId);
  await ensureChainCommissaryTables();
  const done = !!b.done;
  ...
  const ex: any[] = await (prisma as any).$queryRawUnsafe(`SELECT id FROM public."CommissaryProduction" WHERE chain_id=$1 AND order_date=$2 AND item_key=$3`, chainId, date, itemKey).catch(() => []);
  if (ex.length) {
    await (prisma as any).$executeRawUnsafe(
      `UPDATE public."CommissaryProduction" SET done=$1, done_by=CASE WHEN $1 THEN $2 ELSE NULL END, done_at=CASE WHEN $1 THEN NOW() ELSE NULL END${note !== undefined ? ', note=$5' : ''}, "updatedAt"=NOW() WHERE id=$4`,
      ...(note !== undefined ? [done, who, ex[0].id, note] : [done, who, ex[0].id]),
    ).catch(() => {});
  } else {
    await (prisma as any).$executeRawUnsafe(
      `INSERT INTO public."CommissaryProduction"(id, chain_id, order_date, item_key, done, done_by, done_at, note) VALUES (gen_random_uuid()::text, $1, $2, $3, $4, CASE WHEN $4 THEN $5 ELSE NULL END, CASE WHEN $4 THEN NOW() ELSE NULL END, $6)`,
      chainId, date, itemKey, done, who, note ?? null,
    ).catch(() => {});
  }
  return { ok: true };
});
...
registerFn('resetCommissaryProduction', async ({ user, body }) => {
  ...
  const chainId = String(b.chain_id || '');
  const date = String(b.order_date || '').slice(0, 10);
  if (!chainId || !/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error('chain_id + order_date required');
  await assertChainOwner(user, chainId);
  await ensureChainCommissaryTables();
  await (prisma as any).$executeRawUnsafe(`DELETE FROM public."CommissaryProduction" WHERE chain_id=$1 AND order_date=$2`, chainId, date).catch(() => {});
  return { ok: true };
});
...
registerFn('setChainCommissaryItemPricing', async ({ user, body }) => {
  ...
  if (b.stock_qty !== undefined) {
    ...
    await (prisma as any).$executeRawUnsafe(`UPDATE public."CommissaryCatalogItem" SET stock_qty=$1, "updatedAt"=NOW() WHERE chain_id=$2 AND item_key=$3`, s, chainId, itemKey).catch(() => {});
  }
  if (b.due_date !== undefined) {
    const d = /^\d{4}-\d{2}-\d{2}$/.test(String(b.due_date)) ? String(b.due_date).slice(0, 10) : null;
    await (prisma as any).$executeRawUnsafe(`UPDATE public."CommissaryCatalogItem" SET due_date=$1, "updatedAt"=NOW() WHERE chain_id=$2 AND item_key=$3`, d, chainId, itemKey).catch(() => {});
  }
...
async function resolveBranchPhone(chainId: string, branchSlug: string): Promise<string | null> {
  const m: any[] = await (prisma as any).$queryRawUnsafe(`SELECT phone FROM public."ChainMember" WHERE chain_id=$1 AND slug=$2 LIMIT 1`, chainId, branchSlug).catch(() => []);
  if (m[0]?.phone) return String(m[0].phone);
  const t: any[] = await (prisma as any).$queryRawUnsafe(`SELECT owner_phone FROM public."Tenant" WHERE slug=$1 LIMIT 1`, branchSlug).catch(() => []);
...
registerFn('setBranchPhone', async ({ user, body }) => {
  ...
  if (!chainId || !slug) throw new Error('chain_id + slug required');
  await assertChainOwner(user, chainId);
  await ensureChainCommissaryTables();
  const phone = b.phone != null ? String(b.phone).replace(/[^\d+]/g, '').slice(0, 20) : '';
  await (prisma as any).$executeRawUnsafe(`UPDATE public."ChainMember" SET phone=$1 WHERE chain_id=$2 AND slug=$3`, phone || null, chainId, slug).catch(() => {});
...
registerFn('approveChainOrder', async ({ user, body }) => {
  ...
  await (prisma as any).$executeRawUnsafe(`UPDATE public."CommissaryChainOrderLine" SET rejected=false, reject_reason=NULL WHERE order_id=$1`, orderId).catch(() => {});
  const rejectedNames: string[] = [];
  for (const rj of rejections) {
    const key = String(rj.item_key || ''); if (!key) continue;
    const reason = rj.reason ? String(rj.reason).slice(0, 300) : null;
    await (prisma as any).$executeRawUnsafe(`UPDATE public."CommissaryChainOrderLine" SET rejected=true, reject_reason=$3 WHERE order_id=$1 AND item_key=$2`, orderId, key, reason).catch(() => {});
    const nm: any[] = await (prisma as any).$queryRawUnsafe(`SELECT name FROM public."CommissaryChainOrderLine" WHERE order_id=$1 AND item_key=$2 LIMIT 1`, orderId, key).catch(() => []);
    rejectedNames.push(`${nm[0]?.name || key}${reason ? ` (${reason})` : ''}`);
  }
  const status = rejections.length ? 'approved_partial' : 'approved';
  await (prisma as any).$executeRawUnsafe(`UPDATE public."CommissaryChainOrder" SET status=$1, eta=$2, approved_at=NOW(), approved_by=$3, "updatedAt"=NOW() WHERE id=$4`, status, eta, who, orderId).catch(() => {});

  // Notify the branch (WhatsApp) — the in-app status is updated regardless.
  let notify: any = { sent: false };
  if (b.notify !== false) {
    const phone = await resolveBranchPhone(chainId, branchSlug);
    if (!phone) notify = { sent: false, error: 'no_phone' };
    else {
      ...
      if (eta) msg += `\n🕐 מוכן בערך: *${eta}*`;
      if (rejectedNames.length) msg += `\n⚠️ פריטים שלא נכנסו:\n• ${rejectedNames.join('\n• ')}`;
      try { await sendWhatsApp(phone, msg); notify = { sent: true, to: phone }; }
      catch (e: any) { notify = { sent: false, error: String(e?.message || e).slice(0, 120) }; }
    }
  }
...
registerFn('notifyBranchOrderReady', async ({ user, body }) => {
  ...
  try { await sendWhatsApp(phone, msg); return { ok: true, status: 'ready', sent: true, sent_to: phone }; }
  catch (e: any) { return { ok: true, status: 'ready', sent: false, error: String(e?.message || e).slice(0, 120) }; }
});
