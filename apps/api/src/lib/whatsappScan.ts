...
async function storePendingScan(fromPhone: string, scan: ScanResult, preview: string): Promise<void> {
  await (prisma as any).whatsAppMessage.create({
    data: {
      ...
      raw: { pending_scan: { classification: scan.classification, parsed: scan.parsed } } as any,
      is_read: false,
    },
  }).catch(() => {});
}

function buildScanPreview(scan: ScanResult): string {
  const cfg: Record<string, { rowsKey: string; render: (r: any) => string }> = {
...
export async function tryHandleAdminScanMedia(mediaUrl: string, fromPhone: string): Promise<string | null> {
...
export async function tryConfirmPendingScan(fromPhone: string, body: string): Promise<string | null> {
  ...
  if (isCancel) return '❌ בוטל. הפריטים לא יובאו.';

  const ps = pending.raw?.pending_scan;
  if (!ps?.classification || !ps?.parsed) return '🤔 לא מצאתי מה לייבא. שלח את המסמך שוב.';
  try {
