...
export type CardCharge = {
  billing_date: string;        // when the account is charged
  transaction_date: string;    // when the purchase happened
  merchant: string;
  deal_amount: number;         // the purchase
  charged_amount: number;      // billed this cycle — differs when paid in instalments
  currency: string;
  hash: string;
};
...
function currencyOf(sheetName: string): string {
  const s = String(sheetName || '');
  if (/דולר|usd|\$/i.test(s)) return 'USD';
  if (/אירו|euro|eur|€/i.test(s)) return 'EUR';
  if (/שקל|ils|₪/i.test(s)) return 'ILS';
  return 'ILS';
}

/** Parse one sheet's grid. Sheets that are not charge tables yield nothing. */
export function parseCardGrid(grid: string[][], sheetName: string): CardCharge[] {
...
export function summarizeCard(charges: CardCharge[]) {
  const byMerchant = new Map<string, { total: number; count: number; currency: string }>();
  const byMonth = new Map<string, number>();
  for (const c of charges) {
    const k = `${c.currency}|${c.merchant}`;
    const m = byMerchant.get(k) || { total: 0, count: 0, currency: c.currency };
    m.total += c.charged_amount; m.count++;
    byMerchant.set(k, m);
    if (c.currency === 'ILS') {
 ...
 */
export function matchMerchantsToSuppliers(
  merchants: { merchant: string; total: number }[],
  suppliers: { id: string; name: string }[],
): { merchant: string; total: number; supplier_id: string; supplier_name: string }[] {
  ...
  for (const m of merchants) {
    ...
    for (const s of suppliers) {
      ...
      let score = 0;
      if (longer.startsWith(shorter)) score = shorter.length;
      else if (longer.includes(shorter)) score = shorter.length - 1;
      if (score >= 6 && (!best || score > best.score)) best = { s, score };
    }
    if (best) {
 ...
 */
export async function parseCardFile(buf: Buffer, filename = ''): Promise<CardParseResult> {
