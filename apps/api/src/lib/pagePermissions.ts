...
export async function ensurePermissionTiers(): Promise<void> {
  ...
  await (prisma as any).$executeRawUnsafe(`ALTER TABLE "PermissionTier" ADD COLUMN IF NOT EXISTS "allowed_pages" JSONB`).catch(() => {});
  await (prisma as any).$executeRawUnsafe(`ALTER TABLE "Employee" ADD COLUMN IF NOT EXISTS "permission_tier_id" TEXT`).catch(() => {});
  _permEnsured = true;
}
...
export async function resolveUserTier(user: any): Promise<ResolvedTier> {
  if (String(user?.role) === 'owner') {
    return { is_owner: true, tier: null, allowed_pages: null, source: 'owner' };
  }
  await ensurePermissionTiers();
  const tiers: any[] = await (prisma as any)
    .$queryRawUnsafe(`SELECT * FROM "PermissionTier" ORDER BY "sort" ASC`).catch(() => []);
  if (!tiers.length) return { is_owner: false, tier: null, allowed_pages: null, source: 'no_tiers' };

  const emps: any[] = user?.email
    ? await (prisma as any).$queryRawUnsafe(
        `SELECT id, role, permission_tier_id, positions FROM "Employee" WHERE lower(email)=lower($1) LIMIT 2`,
        user.email,
      ).catch(() => [])
    : [];
...
function logDeny(fn: string, user: any, why: string) {
...
export async function requireBackOffice(user: any, fnName: string, pageName?: string): Promise<void> {
  if (!user?.id) { logDeny(fnName, user, 'unauthenticated'); throw new Error('unauthorized'); }
  if (!isBackOfficeRole(user.role)) { logDeny(fnName, user, 'not_back_office'); throw new Error('forbidden'); }
  if (pageName) {
    try { await requirePageAccess(user, pageName); }
    catch (e) { logDeny(fnName, user, `page:${pageName}`); throw e; }
  }
}
...
export async function requireStaff(user: any, fnName: string, pageName?: string): Promise<void> {
  if (!user?.id) { logDeny(fnName, user, 'unauthenticated'); throw new Error('unauthorized'); }
  if (pageName) {
    try { await requirePageAccess(user, pageName); }
    catch (e) { logDeny(fnName, user, `page:${pageName}`); throw e; }
...
export async function requirePageAccess(user: any, pageName: string): Promise<void> {
  const r = await resolveUserTier(user).catch(() => null);
  if (r?.is_owner) return;
  if (!r || r.allowed_pages === null) return;
  if (!r.allowed_pages.includes(pageName)) throw new Error('forbidden_page');
}
