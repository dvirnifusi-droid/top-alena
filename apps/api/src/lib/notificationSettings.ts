...
import { NOTIFICATIONS, byKey, applyTokens, type NotifDef } from './notificationRegistry.js';
...
export interface NotifRow {
  ...
  schedule_json: { time?: string; slots?: Record<string, string> } | null;
}
...
let _cache: { at: number; rows: Map<string, NotifRow> } | null = null;
const CACHE_MS = 60_000;

async function loadRows(): Promise<Map<string, NotifRow>> {
  if (_cache && Date.now() - _cache.at < CACHE_MS) return _cache.rows;
  await ensureTable();
  const rows: any[] = await dbx().$queryRawUnsafe(
    `SELECT key, enabled, custom_text, schedule_json FROM "NotificationSetting"`).catch(() => []);
...
export async function isNotifEnabled(key: string): Promise<boolean> {
  try {
    ...
    if (row && row.enabled !== null) return row.enabled;
...
export async function notifOverrideText(key: string): Promise<string | null> {
  try {
 ...
 */
export async function notifText(
  key: string,
  fallback: string,
...
export async function notifSchedule(
  key: string,
  fallback?: { time?: string; slots?: Record<string, string> },
): Promise<{ time?: string; slots?: Record<string, string> }> {
  const def = byKey(key)?.defaultSchedule;
  let override: NotifRow['schedule_json'] = null;
  try { override = (await loadRows()).get(key)?.schedule_json ?? null; } catch { /* fall through */ }
  return override || def || fallback || {};
}

/** The wall-clock "HH:mm" a per-weekday message should fire on (Israel dow 0..6). */
export async function notifSlot(
  key: string,
  dow: number,
  fallbackSlots: Record<string, string>,
): Promise<string | null> {
...
export async function notifTime(key: string, fallback: string): Promise<string> {
  const sched = await notifSchedule(key, { time: fallback });
  return sched.time || fallback;
}

// ── admin surface (used by the settings functions) ──────────────────────────

export interface NotifSettingView extends NotifDef {
  enabled: boolean;                    // effective
  custom_text: string | null;          // override only (null = using default)
  schedule: { time?: string; slots?: Record<string, string> }; // effective
  overridden: { enabled: boolean; text: boolean; schedule: boolean };
}

/** The full list for the settings page: registry merged with any overrides. */
export async function listNotifSettings(): Promise<NotifSettingView[]> {
...
export interface NotifPatch {
  enabled?: boolean;
  custom_text?: string | null;   // '' or null clears the override
  schedule_json?: { time?: string; slots?: Record<string, string> } | null;
}

/** Upsert one message's overrides. Only the fields present in `patch` change. */
export async function setNotifSetting(key: string, patch: NotifPatch, userId?: string): Promise<void> {
  const def = byKey(key);
  if (!def) throw new Error('unknown notification key');
  await ensureTable();

  // Merge onto the existing row so an unspecified field is preserved.
  const existing = (await dbx().$queryRawUnsafe(
    `SELECT enabled, custom_text, schedule_json FROM "NotificationSetting" WHERE key = $1`, key
  ).catch(() => []))?.[0] || {};

  let enabled: boolean | null = existing.enabled ?? null;
  if (patch.enabled !== undefined) enabled = !!patch.enabled;

  let text: string | null = existing.custom_text ?? null;
  ...
  await dbx().$executeRawUnsafe(
    `INSERT INTO "NotificationSetting" (key, enabled, custom_text, schedule_json, updated_at, updated_by)
    ...
    key, enabled, text, schedule ? JSON.stringify(schedule) : null, userId || null,
  ).catch((e: any) => { throw new Error('save failed: ' + (e?.message || e)); });

  invalidate();
}

/** Revert one message entirely to its registry default (removes the row). */
export async function resetNotifSetting(key: string): Promise<void> {
  await ensureTable();
...
function sanitizeSchedule(
  def: NotifDef,
  s: { time?: string; slots?: Record<string, string> },
): { time?: string; slots?: Record<string, string> } | null {
