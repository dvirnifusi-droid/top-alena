...
export async function getBusinessContext(): Promise<string> {
  const now = Date.now();
  if (_cache && now - _cache.at < TTL_MS) return _cache.text;
  ...
  if (profile?.business_type) lines.push(`סוג עסק: ${profile.business_type}`);
  if (profile?.cuisine_style) lines.push(`סגנון מטבח: ${profile.cuisine_style}`);
  if (profile?.city || profile?.address) {
    const loc = [profile.address, profile.city].filter(Boolean).join(', ');
    lines.push(`מיקום: ${loc}`);
  }
  if (profile?.phone) lines.push(`טלפון: ${profile.phone}`);
  if (profile?.target_audience) lines.push(`קהל יעד: ${profile.target_audience}`);
  if (profile?.description) lines.push(`תיאור: ${profile.description}`);
  if (profile?.menu_description) lines.push(`תפריט: ${profile.menu_description}`);
  ...
  if (oh && typeof oh === 'object') {
    const days: Record<string, string> = { sun: 'א', mon: 'ב', tue: 'ג', wed: 'ד', thu: 'ה', fri: 'ו', sat: 'ש' };
    const parts: string[] = [];
    for (const [k, v] of Object.entries(oh as Record<string, any>)) {
      const label = days[k] || k;
      if (v?.open && v?.close) parts.push(`${label} ${v.open}-${v.close}`);
      else if (v?.closed) parts.push(`${label} סגור`);
    }
    if (parts.length) lines.push(`שעות פעילות: ${parts.join(', ')}`);
  }
  if (profile?.primary_goals) lines.push(`מטרות עסקיות: ${profile.primary_goals}`);
  if (profile?.current_challenges) lines.push(`אתגרים נוכחיים: ${profile.current_challenges}`);
  if (profile?.brand_persona) lines.push(`טון מותג: ${profile.brand_persona}`);
...
export function invalidateBusinessContextCache(): void {
  _cache = null;
}
