...
async function ownerSummary(text: string) {
  try {
    if (!(await isNotifEnabled('nudge_summary'))) return;
...
async function checkAvailability(cfg: NudgeConfig) {
  ...
  if (!(await isNotifEnabled('nudge_availability'))) return;
  const now = ilParts();
...
async function checkClockIn(cfg: NudgeConfig) {
  ...
  if (!(await isNotifEnabled('nudge_clockin'))) return;
  const now = ilParts();
  ...
  for (const ws of shifts) {
    ...
    for (const a of staff) {
      ...
      if (await sendNudge(emp, body, key, 'clockin')) nudged.push(`${emp.full_name} (${startHHMM})`);
    }
  }
