...
export function buildConsentUrl(statePhone: string): string {
...
export async function saveGoogleTokens(phone: string, payload: {
  ...
  const refresh = payload.refresh_token || (existing?.raw as any)?.refresh_token;
  if (existing) {
    await (prisma as any).whatsAppMessage.update({
      ...
      data: {
        raw: {
          ...
          email: payload.email || (existing.raw as any)?.email,
          calendar_id: (existing.raw as any)?.calendar_id || 'primary',
          tasks_list_id: (existing.raw as any)?.tasks_list_id || '@default',
        } as any,
...
async function loadGoogleTokenRow(phone: string): Promise<any | null> {
  return (prisma as any).whatsAppMessage.findFirst({
    where: { status: 'google_oauth_tokens', contact_phone: phone },
    orderBy: { id: 'desc' },
  }).catch(() => null);
}

export async function isGoogleConnected(phone: string): Promise<boolean> {
  const row = await loadGoogleTokenRow(phone);
  return !!(row && (row.raw as any)?.refresh_token);
}

// Return a usable access_token; refresh if needed. Returns null if not connected.
async function getValidAccessToken(phone: string): Promise<string | null> {
  const row = await loadGoogleTokenRow(phone);
  if (!row) return null;
...
export async function createCalendarEvent(phone: string, params: {
  title: string; whenIso: string; durationMin?: number; description?: string;
}): Promise<{ ok: true; id: string; htmlLink?: string } | { ok: false; error: string }> {
...
export async function createTasksItem(phone: string, params: {
  title: string; notes?: string; due?: string;
}): Promise<{ ok: true; id: string; webViewLink?: string } | { ok: false; error: string }> {
  const token = await getValidAccessToken(phone);
  if (!token) return { ok: false, error: 'not_connected' };
  const row = await loadGoogleTokenRow(phone);
  const tasksListId = (row?.raw as any)?.tasks_list_id || '@default';
  const body: any = { title: params.title, notes: params.notes };
  if (params.due) body.due = new Date(params.due).toISOString();
...
export async function completeTasksItem(phone: string, taskGoogleId: string): Promise<boolean> {
  const token = await getValidAccessToken(phone);
...
export async function pullCalendarEvents(phone: string, daysAhead = 30): Promise<{
  ok: boolean; imported: number; updated: number; skipped: number; error?: string;
}> {
  const token = await getValidAccessToken(phone);
  if (!token) return { ok: false, imported: 0, updated: 0, skipped: 0, error: 'not_connected' };
  const row = await loadGoogleTokenRow(phone);
  const calendarId = (row?.raw as any)?.calendar_id || 'primary';
  ...
  if (!res.ok) return { ok: false, imported: 0, updated: 0, skipped: 0, error: `pull_${res.status}` };
  const data: any = await res.json();
  ...
  for (const ev of items) {
    ...
    if (match) {
      ...
      const oldAt = (match.raw as any)?.event_at;
      const newTitle = ev.summary || '(ללא כותרת)';
      const oldTitle = (match.raw as any)?.title;
      if (newAt !== oldAt || newTitle !== oldTitle) {
        await (prisma as any).whatsAppMessage.update({
          where: { id: match.id },
          data: {
            raw: { ...(match.raw as any), event_at: newAt, title: newTitle, notified_lead: false, notified_start: false } as any,
          },
        });
        updated++;
      } else skipped++;
...
export async function pullAllConnectedCalendars(): Promise<Array<{ phone: string; result: any }>> {
  const tokens: any[] = await (prisma as any).whatsAppMessage.findMany({
    where: { status: 'google_oauth_tokens' },
    take: 50,
  });
  const out: Array<{ phone: string; result: any }> = [];
  for (const t of tokens) {
    const r = await pullCalendarEvents(t.contact_phone, 30).catch((e: any) => ({ ok: false, error: e?.message }));
