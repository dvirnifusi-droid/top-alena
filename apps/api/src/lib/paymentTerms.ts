...
export type ParsedTerms = {
  kind: 'immediate' | 'net_days' | 'eom_plus';
  days: number;
  label: string;
  occasional: boolean;
};
...
export function parsePaymentTerms(raw: any, opts: { occasional?: boolean } = {}): ParsedTerms {
...
export function dueDateFor(invoiceDate: Date | string, terms: ParsedTerms): Date {
  const d = invoiceDate instanceof Date ? new Date(invoiceDate) : new Date(invoiceDate);
  if (isNaN(d.getTime())) return new Date();
  if (terms.kind === 'immediate') return d;
