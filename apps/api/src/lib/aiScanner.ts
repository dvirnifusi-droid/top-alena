...
export type ScanType = 'menu' | 'checklist' | 'employees' | 'suppliers' | 'order_list' | 'recipe';
...
export const SCAN_PARSE_SPECS: Record<ScanType, { label: string; rowsKey: string; prompt: string; schema: any }> = {
  menu: {
    label: 'תפריט',
    rowsKey: 'items',
    prompt:
      `זהו תפריט מסעדה (אוכל / שתייה / קוקטיילים). חלץ רשימת פריטים מובנית.\n` +
  ...
  checklist: {
    ...
    rowsKey: 'items',
    prompt:
      `זו רשימת משימות / צ׳קליסט תפעולי. חלץ כותרת קצרה לרשימה ואת הפריטים לפי הסדר.\n` +
  ...
  employees: {
    ...
    rowsKey: 'employees',
    prompt:
      `זו רשימת עובדים. חלץ לכל עובד: שם מלא, טלפון (אם יש), תפקיד (אם יש), מחלקה (אם יש).\n` +
  ...
  order_list: {
    ...
    rowsKey: 'items',
    prompt:
      `זו רשימת הזמנות / מוצרים לספק. חלץ שם לרשימה ואת הפריטים.\n` +
  ...
  recipe: {
    ...
    rowsKey: 'ingredients',
    prompt:
      `זהו מתכון מטבח. חלץ בצורה מובנית:\n` +
      ...
      `- ingredients: מערך — חובה לכלול כל שורת רכיב שמופיעה בטקסט. לכל רכיב: name (שם חומר גלם בעברית, פשוט וכללי), qty (מספר בלבד — למשל 2), unit (יחידת המידה כפי שכתובה: ק״ג/גרם/ליטר/מ״ל/יח׳ וגם מידות ביתיות כמו כוס/כף/כפית/חבילה/קופסה).\n` +
      `דוגמה: "2 כוסות חלב" → {"name":"חלב","qty":2,"unit":"כוס"}. "מלח" (בלי כמות) → {"name":"מלח"}.\n` +
      `אל תשאיר את ingredients ריק אם יש רכיבים בטקסט. שמור שמות פשוטים ("קמח תירס", "חלב", "חמאה", "פרמזן", "מלח"). אל תמציא כמויות שלא כתובות — אם חסר, השאר qty ריק אך כלול את הרכיב.`,
    schema: {
      type: 'object',
      properties: {
        name: { type: 'string' },
...
const KNOWN_TYPES = Object.keys(SCAN_PARSE_SPECS) as ScanType[];

// Classify unknown input into one of the known domains (or invoice/unknown).
export async function classifyScanContent(fileUrls: string[], text?: string) {
  const options = [...KNOWN_TYPES, 'invoice']; // recipe is already in KNOWN_TYPES
  const res: any = await invokeLLM({
    prompt:
      `אתה מסווג מסמכים במערכת ניהול מסעדה. סווג את התוכן לאחת מהקטגוריות:\n` +
  ...
  if (!options.includes(type)) type = 'unknown';
...
export async function scanContent(opts: { fileUrls?: string[]; text?: string; hint?: string }): Promise<ScanResult> {
  ...
  if (!urls.length && !freeText) throw new Error('file_url or text required');

  let classification: string, confidence: number, preview_text: string;
  const validHint = opts.hint && (SCAN_PARSE_SPECS as any)[opts.hint] ? opts.hint : null;
  if (validHint) {
    classification = validHint; confidence = 1; preview_text = '';
  } else {
    const c = await classifyScanContent(urls, freeText);
    classification = c.type; confidence = c.confidence; preview_text = c.preview_text;
  }

  const spec = (SCAN_PARSE_SPECS as any)[classification];
  if (!spec) {
  ...
  const parsed: any = await invokeLLM({
...
const synthEmail = (p: string) => `${p}-${Math.random().toString(36).slice(2, 8)}@import.local`;

function mapDept(d: any): string | null {
  const s = String(d || '').toLowerCase().trim();
  if (/kitchen|מטבח|טבח|שף|בישול/.test(s)) return 'kitchen';
  if (/bar|בר|ברמן|בריסט/.test(s)) return 'bar';
  if (/floor|אולם|שירות|מלצר|מארח|dining|service/.test(s)) return 'floor';
  return null;
}

function mapSupplierCat(c: any): string {
  const s = String(c || '').toLowerCase().trim();
  if (/service|שירות/.test(s)) return 'services';
  if (/equip|ציוד/.test(s)) return 'equipment';
  if (/cater|קייטר/.test(s)) return 'catering';
  return 'materials';
}

export async function importScanned(classification: string, parsed: any): Promise<ImportSummary> {
...
async function importMenu(parsed: any): Promise<ImportSummary> {
...
async function importChecklist(parsed: any): Promise<ImportSummary> {
  ...
  const items = raw
    ...
    .filter(Boolean);
  if (!items.length) return { created: 0, skipped: 0, label: 'צ׳קליסט', message: 'לא נמצאו פריטים לצ׳קליסט.' };
...
async function importEmployees(parsed: any): Promise<ImportSummary> {
...
async function importSuppliers(parsed: any): Promise<ImportSummary> {
...
async function ensurePrepTables(): Promise<void> {
  if (_prepTablesEnsured) return;
...
async function importOrderList(parsed: any): Promise<ImportSummary> {
  const rows = Array.isArray(parsed?.items) ? parsed.items : [];
  const items = rows.filter((it: any) => String(it?.name || '').trim());
  if (!items.length) return { created: 0, skipped: 0, label: 'רשימת הזמנות', message: 'לא נמצאו פריטים ברשימה.' };
  ...
  const mx: any[] = await db.$queryRawUnsafe(`SELECT COALESCE(MAX("sort"),0)+1 AS s FROM "PrepList"`).catch(() => [{ s: 0 }]);
  await db.$executeRawUnsafe(
    `INSERT INTO "PrepList" ("id","name","sort","list_type","updatedAt") VALUES ($1,$2,$3,'order',NOW())`,
    listId, listName, Number(mx[0]?.s) || 0,
  );

  let sort = 0;
  for (const it of items) {
    await db.$executeRawUnsafe(
      `INSERT INTO "PrepItem" ("id","name","category","unit","target","have","to_prep","done","list_id","sort","updatedAt")
       VALUES ($1,$2,$3,$4,$5,'',false,false,$6,$7,NOW())`,
      randomUUID(),
      String(it?.name).trim().slice(0, 200),
      it?.supplier ? String(it.supplier).slice(0, 80) : null,
      it?.unit ? String(it.unit).slice(0, 40) : null,
      it?.qty != null ? String(it.qty).slice(0, 40) : '',
      listId,
      sort++,
    );
  }
...
async function ensureRecipeTables(): Promise<void> {
  if (_recipeTablesEnsured) return;
...
const normIng = (s: any) => String(s || '').toLowerCase().trim().replace(/["'׳״.,\-+()/\\]/g, '').replace(/\s+/g, ' ');

async function importRecipe(parsed: any): Promise<ImportSummary> {
  const name = String(parsed?.name || '').trim();
  if (!name) return { created: 0, skipped: 0, label: 'מתכון', message: 'לא זוהה שם מתכון.' };
  ...
  await db.$executeRawUnsafe(
    `INSERT INTO "Recipe" ("id","kind","name","yield_qty","yield_unit","createdAt","updatedAt") VALUES ($1,$2,$3,$4,$5,NOW(),NOW())`,
    recipeId, kind, name.slice(0, 200), yieldQty, yieldUnit,
  );
  ...
  const aliases: any[] = await db.$queryRawUnsafe(
    `SELECT a.alias, i.id, i.price_per_unit, i.waste_percent FROM "IngredientAlias" a JOIN "Ingredient" i ON i.id=a.ingredient_id`).catch(() => []);
  const ings: any[] = await db.$queryRawUnsafe(`SELECT id, name, price_per_unit, waste_percent FROM "Ingredient"`).catch(() => []);

  const matchIngredient = async (iname: string, unit: string) => {
    const n = normIng(iname);
    for (const a of aliases) if (normIng(a.alias) === n) return a;
    for (const i of ings) if (normIng(i.name) === n) return i;
    // fuzzy — token overlap
    ...
    const id = rid('ing');
    await db.$executeRawUnsafe(
      `INSERT INTO "Ingredient" ("id","name","unit","waste_percent","createdAt","updatedAt") VALUES ($1,$2,$3,0,NOW(),NOW())`,
      id, iname.slice(0, 160), unit,
    );
  ...
  for (const ing of ingredients) {
    ...
    const m: any = await matchIngredient(iname, unit);
    if (m._new) newIng++;
    ...
    if (cost) total += cost;
    await db.$executeRawUnsafe(
      `INSERT INTO "RecipeIngredient" ("id","recipe_id","ingredient_id","qty","unit","cost_at_import","createdAt") VALUES ($1,$2,$3,$4,$5,$6,NOW())`,
      rid('ri'), recipeId, m.id, qty, unit, cost,
    );
    lineCount++;
  }
  ...
  try {
    const load: any = await import('../functions/load.js');
    if (typeof load.recomputeAllRecipeCosts === 'function') await load.recomputeAllRecipeCosts();
