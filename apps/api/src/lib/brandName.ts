...
let _cache: { name: string; at: number } | null = null;
const TTL_MS = 60_000;
...
export async function getBrandName(): Promise<string> {
  ...
  try {
    const p: any = await (prisma as any).restaurantProfile.findFirst({
 ...
 */
export function renderBrand(template: string, brand: string): string {
 ...
 */
export async function renderWithBrand(template: string): Promise<string> {
