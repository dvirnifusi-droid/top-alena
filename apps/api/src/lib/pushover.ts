...
async function mirrorToWhatsApp(title: string, message: string) {
  try {
    ...
    for (const phone of phones) {
      try { await notifyOwner(phone, title || 'התראה', message); }
...
export async function pushoverToAdmins(title: string, message: string) {
  ...
  // who never opted in.
  const users = await (prisma as any).user.findMany({ where: { role: { in: ['admin', 'owner'] } } })
    .catch(() => []);
  const adminEmails = new Set(
    users.map((u: any) => String(u.email || '').toLowerCase()).filter(Boolean),
  );
  ...
  for (const e of employees) {
    ...
    if (byUser || byRole) keys.add(key);
  }

  if (!keys.size) {
    // Loud, because the previous failure mode was total silence with no trace.
    console.warn(
      `[pushover] NO RECIPIENTS — ${users.length} admin/owner users, ` +
