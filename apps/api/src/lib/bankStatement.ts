...
export type BankTx = {
  ...
  value_date: string;      // YYYY-MM-DD — תאריך ערך, when the money really moves
  description: string;
  counterparty: string | null; // the other side, when the bank names it
  amount: number;          // + money in, - money out
  balance: number | null;
  reference: string;
  category: string;
  hash: string;            // stable dedupe key across re-uploads
};

export type ParseResult = {
  ok: boolean;
  format: string;          // html | csv | xlsx | unknown
  bank: string | null;     // best-effort detection, informational only
  account: string | null;
  currency: string;
...
const isHtml = (s: string) => /<table[\s>]/i.test(s) || /<html[\s>]/i.test(s);
...
function gridFromHtml(s: string): string[][] {
  const rows: string[][] = [];
...
function gridFromCsv(s: string): string[][] {
...
type Cols = Partial<Record<keyof typeof SYN, number>>;

function findHeader(grid: string[][]): { row: number; cols: Cols } | null {
  ...
  for (let r = 0; r < Math.min(grid.length, 60); r++) {
    ...
    grid[r].forEach((cell, i) => {
      ...
      for (const [key, re] of Object.entries(SYN)) {
        if (re.test(t) && (cols as any)[key] === undefined) (cols as any)[key] = i;
      }
    });
    ...
    if (usable && (!best || score > best.score)) best = { row: r, cols, score };
  }
...
const p2 = (n: any) => String(Number(n)).padStart(2, '0');
...
const RULES: { re: RegExp; inCat?: string; outCat?: string }[] = [
...
export function extractCounterparty(description: string): string | null {
  ...
  if (GENERIC.test(d)) return null;
  return d;
}

/** Hebrew labels + direction, shared by every UI that shows a category. */
export const CATEGORY_LABELS: Record<string, { he: string; dir: 'in' | 'out' | 'both' }> = {
...
function detectBank(text: string, filename = ''): string | null {
  const t = text.slice(0, 4000) + ' ' + filename;
  if (/מזרחי|mizrahi|טפחות|accountactivity/i.test(t)) return 'מזרחי טפחות';
  if (/הפועלים|poalim/i.test(t)) return 'הפועלים';
  if (/לאומי|leumi/i.test(t)) return 'לאומי';
  if (/דיסקונט|discount/i.test(t)) return 'דיסקונט';
  if (/הבינלאומי|fibi/i.test(t)) return 'הבינלאומי';
  if (/מרכנתיל/i.test(t)) return 'מרכנתיל';
  if (/ירושלים/i.test(t)) return 'ירושלים';
  if (/יהב/i.test(t)) return 'יהב';
  if (/one\s*zero|וואן\s*זירו/i.test(t)) return 'OneZero';
  return null;
}
...
export function parseBankStatement(raw: string, filename = ''): ParseResult {
...
function parseGrid(grid: string[][], format: string, filename = ''): ParseResult {
  ...
  if (cols.description === undefined) warnings.push('לא נמצאה עמודת תיאור — הסיווג האוטומטי יהיה חלקי');

  const at = (r: string[], i?: number) => (i === undefined ? '' : (r[i] ?? ''));
  ...
  for (let r = hRow + 1; r < grid.length; r++) {
    ...
    if (!date) { if (row.some((c) => c)) skipped++; continue; }
    ...
    if (!amount) continue; // a zero row is a separator, not a transaction
  ...
  if (skipped > 3) warnings.push(`${skipped} שורות דולגו (ללא תאריך תקין) — בדרך כלל כותרות וסיכומים`);
  if (!txs.length) warnings.push('לא נמצאו תנועות');
 ...
 */
export async function xlsxSheets(buf: Buffer): Promise<{ name: string; grid: string[][] }[]> {
...
async function gridFromXlsx(buf: Buffer): Promise<string[][]> {
 ...
 */
export async function parseBankFile(buf: Buffer, filename = ''): Promise<ParseResult> {
  ...
  if ((text.match(/�/g) || []).length > 5) {
    try { text = new TextDecoder('windows-1255' as any).decode(buf); } catch { /* keep utf8 */ }
  }
  return parseBankStatement(text, filename);
}

/** Roll transactions up per month and per category — the import summary. */
export function summarize(txs: BankTx[]) {
