...
function defaultFactor(titleEn: string, titleHe = ''): { factor: number; closed: boolean } {
  ...
  if (/מעשר בהמה|rosh hashana la.?behemah|לאילנות/.test(t)) return { factor: 1, closed: false };
  // Order matters: every "erev" test must precede the festival it belongs to.
  if (/erev yom kippur|ערב יום כיפור/.test(t)) return { factor: 0.5, closed: false };
  if (/yom kippur|יום כיפור/.test(t)) return { factor: 0, closed: true };   // the country stops
  if (/tish.?a b.av|תשעה באב/.test(t)) return { factor: 0.2, closed: false };
  ...
  if (/ch.?.?m|chol hamoed|חוה|חול המועד/.test(t)) return { factor: 1.4, closed: false };
  if (/purim|פורים/.test(t)) return { factor: 1.5, closed: false };
  if (/yom haatzmaut|independence|יום העצמאות/.test(t)) return { factor: 1.3, closed: false };
 ...
 */
export async function syncHolidayCalendar(year: number): Promise<{ added: number; total: number }> {
  await ensureTable();
  ...
  try {
    ...
    items = (data.items || []).filter((i: any) => i.category === 'holiday');
  } catch {
  ...
  for (const it of items) {
    ...
    try {
      await dbx().$executeRawUnsafe(
        `INSERT INTO "HolidayCalendar" (holiday_date, name, category, revenue_factor, closed)
         VALUES ($1::date,$2,$3,$4,$5)
         ON CONFLICT (holiday_date) DO UPDATE SET
           name = EXCLUDED.name, category = EXCLUDED.category,
           revenue_factor = EXCLUDED.revenue_factor, closed = EXCLUDED.closed
         WHERE "HolidayCalendar".edited = false`,
        date, name, String(it.subcat || ''), factor, closed);
      added++;
    } catch { /* one bad row must not stop the sync */ }
  }
  const c: any[] = await dbx().$queryRawUnsafe(
    `SELECT COUNT(*)::int c FROM "HolidayCalendar"`).catch(() => []);
  return { added, total: Number(c[0]?.c) || 0 };
}

export async function loadHolidays(fromKey: string, toKey: string): Promise<Map<string, Holiday>> {
  await ensureTable();
  const rows: any[] = await dbx().$queryRawUnsafe(
    `SELECT holiday_date, name, category, revenue_factor, closed, edited
     FROM "HolidayCalendar" WHERE holiday_date >= $1::date AND holiday_date <= $2::date`,
    fromKey, toKey).catch(() => []);
 ...
 */
export function factorForPayout(payoutDate: string, holidays: Map<string, Holiday>, lagDays = 5): number {
