...
export async function buildPersonalReport(fromPhone: string, days = 7): Promise<string> {
  const since = addDays(new Date(), -days);
  const sinceIso = since.toISOString();

  // 1. Tasks completed this week (status=task_done, contact_phone=user)
  const doneTasks: any[] = await (prisma as any).whatsAppMessage.findMany({
    where: {
  ...
  const openTasks: any[] = await (prisma as any).whatsAppMessage.findMany({
    where: { status: 'open_task', contact_phone: fromPhone, is_read: false },
    take: 200,
  }).catch(() => []);

  // 3. Events that already happened this week
  const eventsDone: any[] = await (prisma as any).whatsAppMessage.findMany({
    where: { status: 'event_done', contact_phone: fromPhone, created_at: { gte: since } },
    take: 200,
  }).catch(() => []);

  // 4. Upcoming scheduled events
  const eventsUpcoming: any[] = await (prisma as any).whatsAppMessage.findMany({
  ...
  try {
    const leads: any[] = await (prisma as any).eventLead.findMany({
      where: { updated_date: { gte: sinceIso } as any },
      take: 200,
    });
  ...
  let invoicesPaid = 0;
  try {
    const invs: any[] = await (prisma as any).invoice.findMany({
  ...
  if (openTasks.length) {
    ...
    for (const t of openTasks.slice(0, 5)) {
      const title = (t.raw as any)?.title || t.body || '?';
      lines.push(`  • ${title}`);
    }
    if (openTasks.length > 5) lines.push(`  …ועוד ${openTasks.length - 5}`);
  }
  ...
  if (eventsUpcoming.length) {
    ...
    const sorted = eventsUpcoming
      .map((e: any) => ({ ...e, ts: new Date((e.raw as any)?.event_at || 0).getTime() }))
      .filter((e: any) => e.ts > Date.now())
      .sort((a: any, b: any) => a.ts - b.ts)
      .slice(0, 3);
    for (const e of sorted) {
      const title = (e.raw as any)?.title || '?';
      const at = new Date((e.raw as any)?.event_at).toLocaleString('he-IL', { timeZone: TZ, dateStyle: 'short', timeStyle: 'short' });
      lines.push(`  • ${at} · ${title}`);
    }
  }
