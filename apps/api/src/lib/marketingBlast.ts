...
export function signCustomer(cid: string): string {
  return createHmac('sha256', secret()).update(`unsub:${cid}`).digest('base64url').slice(0, 20);
}

export function verifyCustomerSignature(cid: string, sig: string): boolean {
...
export function optOutUrl(cid: string): string {
 ...
 */
export function memberCardUrl(cid: string): string {
  const base = process.env.PUBLIC_BASE_URL || 'https://topalena.com';
 ...
 */
export function withOptOut(text: string, customerId: string): string {
  if (!customerId) return text;
  if (/להסרה|לביטול דיוור|unsubscribe/i.test(text)) return text;   // already carries one
