...
registerFn('setCommissaryConfig', async ({ user, body }) => {
    ...
    await ensureCommissaryTables();
    const m = Number(body?.default_markup_pct);
    const markup = Number.isFinite(m) && m >= 0 ? m : 30;
    await prisma.$executeRawUnsafe(`INSERT INTO "CommissaryConfig"(id, default_markup_pct, "updatedAt") VALUES ('default', $1, NOW())
     ON CONFLICT (id) DO UPDATE SET default_markup_pct=EXCLUDED.default_markup_pct, "updatedAt"=NOW()`, markup);
