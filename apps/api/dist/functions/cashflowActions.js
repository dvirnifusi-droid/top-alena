...
registerFn('getCashflowActions', async ({ user, body }) => {
    await guard(user);
    ...
    const actions = [];
    const byCat = (c) => patterns.find((p) => p.category === c);
    ...
    const sup = await dbx().$queryRawUnsafe(`SELECT s.id, s.company_name, s.payment_terms, COALESCE(s.is_occasional,false) AS is_occasional,
    ...
    if (forecast?.has_data && forecast.min_point) {
        ...
        const window = (forecast.days || []).filter((d) => d.date <= minDate
            && Date.parse(minDate) - Date.parse(d.date) <= 10 * 86400_000);
        const candidates = window.flatMap((d) => (d.events || [])
            .filter((e) => e.amount < 0)
            // Payroll and tax cannot be rescheduled; suggesting it would be noise.
            .filter((e) => !/משכורת|מע"מ|מס הכנסה|ביטוח לאומי/.test(e.label))
            .map((e) => ({ date: d.date, label: e.label, amount: e.amount })));
        const biggest = candidates.sort((a, b) => a.amount - b.amount)[0];
 ...
 */
registerFn('autoConfigureCashflow', async ({ user }) => {
    await guard(user);
    const applied = [];
    ...
    if (dates.length >= 2) {
        ...
        const existing = await dbx().$queryRawUnsafe(`SELECT id FROM "CashFlowVatSetting" LIMIT 1`).catch(() => []);
        if (!existing.length) {
            await dbx().$executeRawUnsafe(`INSERT INTO "CashFlowVatSetting" (id, period, payment_day, amount_mode, enabled)
         VALUES ($1,$2,$3,'learned',true)`, `vat_${Date.now().toString(36)}`, period, Math.min(28, Math.max(1, day)));
...
//# sourceMappingURL=cashflowActions.js.map