...
export async function loadOpenInvoices() {
    const suppliers = await dbx().$queryRawUnsafe(`SELECT id, company_name, payment_terms, COALESCE(is_occasional,false) AS is_occasional,
     ...
     FROM "Supplier"`).catch(async () => await dbx().$queryRawUnsafe(`SELECT id, company_name, payment_terms, COALESCE(is_occasional,false) AS is_occasional,
    ...
    const rows = await dbx().$queryRawUnsafe(`SELECT id, supplier_id, invoice_date, total_amount,
 ...
 * the owner commits.
 */
registerFn('importBankStatement', async ({ user, body }) => {
    await guard(user);
    const b = (body || {});
...
registerFn('getBankSummary', async ({ user, body }) => {
    await guard(user);
    await ensureTable();
    ...
    const rows = await dbx().$queryRawUnsafe(`SELECT tx_date, description, counterparty, amount, balance, category
...
registerFn('listBankTransactions', async ({ user, body }) => {
    await guard(user);
    await ensureTable();
    const b = (body || {});
    ...
    const args = [];
    if (b.from) {
        args.push(String(b.from));
        where.push(`tx_date >= $${args.length}::date`);
    }
    if (b.to) {
        args.push(String(b.to));
        where.push(`tx_date <= $${args.length}::date`);
    }
    if (b.category) {
        args.push(String(b.category));
        where.push(`category = $${args.length}`);
    }
    if (b.q) {
        args.push(`%${String(b.q)}%`);
        where.push(`(description ILIKE $${args.length} OR counterparty ILIKE $${args.length})`);
    }
    args.push(limit);
    const rows = await dbx().$queryRawUnsafe(`SELECT id, tx_date, description, counterparty, amount, balance, reference,
...
registerFn('setBankTxCategory', async ({ user, body }) => {
    await guard(user);
    await ensureTable();
    const b = (body || {});
    const id = String(b.id || '');
    const category = String(b.category || '');
    if (!id || !CATEGORY_LABELS[category])
        throw new Error('bad_request');
    await dbx().$executeRawUnsafe(`UPDATE "BankTransaction" SET category=$2, category_manual=true WHERE id=$1`, id, category);
...
export async function getVatSettingRow() {
    await ensureVatTable();
    const r = await dbx().$queryRawUnsafe(`SELECT period, payment_day, amount_mode, fixed_amount, enabled
...
registerFn('getVatSetting', async ({ user }) => {
    await guard(user);
    const s = await getVatSettingRow();
    // Show what the bank actually paid, so the owner can sanity-check the estimate.
    const hist = await dbx().$queryRawUnsafe(`SELECT tx_date, amount FROM "BankTransaction"
...
registerFn('setVatSetting', async ({ user, body }) => {
    ...
    await dbx().$executeRawUnsafe(`INSERT INTO "CashFlowVatSetting"
       (id, period, payment_day, amount_mode, fixed_amount, enabled)
     VALUES ($1,$2,$3,$4,$5,$6)`, rid(), period, day, mode, fixed, enabled);
 ...
 */
registerFn('reconcileBankTransactions', async ({ user, body }) => {
    await guard(user);
    await ensureTable();
    const b = (body || {});
    ...
    const txRows = await dbx().$queryRawUnsafe(`SELECT b.id, b.tx_date, b.amount, b.description
    ...
    const takenRows = await dbx().$queryRawUnsafe(`SELECT invoice_id FROM "BankTxMatch"`).catch(() => []);
    const taken = new Set(takenRows.map((r) => String(r.invoice_id)));
    ...
    if (apply) {
        for (const m of accepted) {
            for (const invId of m.invoice_ids) {
                try {
                    await dbx().$executeRawUnsafe(`INSERT INTO "BankTxMatch"
               (id, bank_tx_id, invoice_id, supplier_id, amount, method, confidence)
             VALUES ($1,$2,$3,$4,$5,$6,$7)
             ON CONFLICT (bank_tx_id, invoice_id) DO NOTHING`, rid(), m.bank_tx_id, invId, m.supplier_id, Math.abs(m.bank_amount), m.method, m.confidence);
                }
                catch { /* skip the row, keep the run */ }
            }
            await dbx().$executeRawUnsafe(`UPDATE "BankTransaction" SET counterparty=$2 WHERE id=$1 AND (counterparty IS NULL OR counterparty='')`, m.bank_tx_id, m.supplier_name).catch(() => { });
            stored++;
        }
    }
    const sum = (xs, f) => Math.round(xs.reduce((n, x) => n + f(x), 0));
    return {
        ok: true,
        applied: apply,
        stored,
...
registerFn('getReconciliationSummary', async ({ user }) => {
    await guard(user);
    await ensureTable();
    ...
    const rows = await dbx().$queryRawUnsafe(`WITH payments AS (
...
registerFn('unmatchBankTx', async ({ user, body }) => {
    await guard(user);
    await ensureTable();
 ...
 */
registerFn('recategorizeBankTransactions', async ({ user }) => {
    await guard(user);
    await ensureTable();
    const rows = await dbx().$queryRawUnsafe(`SELECT id, description, amount, category FROM "BankTransaction"
...
registerFn('importCardStatement', async ({ user, body }) => {
    await guard(user);
    const b = (body || {});
    if (!b.content_base64)
        throw new Error('no_content');
    const { parseCardFile, summarizeCard } = await import('../lib/cardStatement.js');
    const parsed = await parseCardFile(Buffer.from(String(b.content_base64), 'base64'), String(b.filename || ''));
    if (!parsed.ok)
        return { ok: false, warnings: parsed.warnings, imported: 0 };
...
registerFn('getCardSummary', async ({ user, body }) => {
    await guard(user);
    ...
    const since = new Date(Date.now() - months * 31 * 86400_000).toISOString().slice(0, 10);
    const rows = await dbx().$queryRawUnsafe(`SELECT billing_date, transaction_date, merchant, deal_amount, charged_amount, currency
     FROM "CardCharge" WHERE billing_date >= $1::date ORDER BY billing_date`, since).catch(() => []);
    if (!rows.length)
        return { has_data: false, merchants: [], overlap: [], totals: [] };
    ...
    const s = summarizeCard(charges);
    const sup = await dbx().$queryRawUnsafe(`SELECT id, company_name, COALESCE(paid_by_card,false) AS paid_by_card FROM "Supplier"`).catch(() => []);
    const overlap = matchMerchantsToSuppliers(s.merchants.filter((m) => m.currency === 'ILS'), sup.map((x) => ({ id: String(x.id), name: x.company_name || '' }))).map((o) => ({
        ...o,
        paid_by_card: !!sup.find((x) => String(x.id) === o.supplier_id)?.paid_by_card,
    }));
    return {
        has_data: true,
 ...
 */
registerFn('setSupplierPaidByCard', async ({ user, body }) => {
    await guard(user);
    const { ensureCardTables } = await import('../lib/bankPersist.js');
    await ensureCardTables();
    const b = (body || {});
