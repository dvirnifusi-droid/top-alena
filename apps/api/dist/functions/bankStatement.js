...
registerFn('importBankStatement', async ({ user, body }) => {
    ...
    const existing = hashes.length
        ? await dbx().$queryRawUnsafe(`SELECT hash FROM "BankTransaction" WHERE hash = ANY($1::text[])`, hashes).catch(() => [])
        : [];
    ...
    if (parsed.credit_line != null || parsed.closing_balance != null) {
       ...
       VALUES ($1,$2,$3,$4,$5,$6::date)`, rid(), parsed.bank, parsed.account, parsed.credit_line, parsed.closing_balance, parsed.transactions[parsed.transactions.length - 1].date).catch(() => { });
    }
