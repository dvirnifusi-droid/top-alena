...
export async function computeCapitalForecast(daysIn) {
                ...
                if (isPaymentMonth) {
                    ...
                    if (d > start) {
                        ...
                        events.push({
                            ...
                            source: `דיווח ${periodLabel}, תשלום ב-${vat.payment_day} לחודש · ${vat.amount_mode === 'fixed' ? 'סכום שהוגדר ידנית' : 'הערכה לפי ממוצע התשלומים בעו"ש'}`,
    ...
    const cardCovered = allOpen.filter((i) => i.paid_by_card);
    const invoices = allOpen.filter((i) => !i.paid_by_card);
    ...
    return {
        ...
        card_covered_amount: Math.round(cardCovered.reduce((t, i) => t + i.amount, 0)),
        warnings,
    };
}
