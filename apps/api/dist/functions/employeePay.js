...
async function roleRatesFor(employeeIds) {
    const out = new Map();
    if (!employeeIds.length)
        return out;
    await ensurePayCols();
    const rows = await prisma.$queryRawUnsafe(`SELECT employee_id, role_rates FROM "EmployeePay" WHERE employee_id = ANY($1::text[]) AND role_rates IS NOT NULL`, employeeIds).catch(() => []);
    for (const r of rows)
        if (r.role_rates && typeof r.role_rates === 'object')
            out.set(r.employee_id, r.role_rates);
    return out;
}
...
async function writeRoleRates(employeeId, raw) {
    if (raw === undefined)
        return; // caller didn't touch role rates → leave as-is
    await ensurePayCols();
    let clean = null;
    ...
    await prisma.$executeRawUnsafe(`UPDATE "EmployeePay" SET "role_rates" = $1::jsonb WHERE employee_id = $2`, clean === null ? null : JSON.stringify(clean), employeeId).catch(() => { });
}
...
registerFn('getEmployeePay', async ({ body, user }) => {
    ...
    const positions = Array.isArray(emp?.positions)
        ? emp.positions.map((p) => p?.position_name).filter(Boolean)
        : (emp?.role ? [emp.role] : []);
...
registerFn('listEmployeePay', async ({ user }) => {
    ...
    return employees
        ...
        const positions = Array.isArray(e.positions)
            ? [...new Set(e.positions.map((p) => p?.position_name).filter(Boolean))]
            : (e.role ? [e.role] : []);
        return {
...
registerFn('setEmployeePayBulk', async ({ body, user }) => {
    ...
    for (const r of rows) {
        ...
        try {
            ...
            if (r.role_rates !== undefined)
                await writeRoleRates(employeeId, r.role_rates);
            saved++;
        }
        catch (e) {
