...
registerFn('getLiveCashFlow', async ({ body, user }) => {
    ...
    const days = Math.min(120, Math.max(7, parseInt(String(body?.days || 30))));
    const today = new Date();
    const rangeEnd = new Date(today.getTime() + days * DAY_MS);
    const setting = await prisma.cashFlowSetting.findFirst({ orderBy: { updatedAt: 'desc' } }).catch(() => null);
    ...
    const reports = await prisma.shiftEndReport.findMany({
    ...
    await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "CashFlowDailyRevenue" (
       "date" TEXT PRIMARY KEY,
       "amount" DOUBLE PRECISION NOT NULL,
       "note" TEXT,
       "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP
     )`).catch(() => { });
    const manualRows = await prisma.$queryRawUnsafe(`SELECT "date", "amount" FROM "CashFlowDailyRevenue"`).catch(() => []);
    for (const m of manualRows) {
        const k = String(m.date).slice(0, 10);
        if (!daily.has(k) || Number(daily.get(k)) <= 0)
            daily.set(k, Number(m.amount) || 0);
    }
    ...
    const invoicesRaw = await prisma.$queryRawUnsafe(`SELECT id, invoice_date, total_amount, payment_status, supplier_id,
            (CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns
                               WHERE table_name='Invoice' AND column_name='due_date')
                  THEN due_date ELSE NULL END) AS due_date
     FROM "Invoice" WHERE COALESCE(status,'') <> 'rejected' AND invoice_date >= $1`, openingDate).catch(async () => await prisma.invoice.findMany({
    ...
    const costs = await prisma.recurringCost.findMany({ where: { active: true } }).catch(() => []);
...
async function buildPayrollEntries(today, rangeEnd) {
    const dbx = prisma;
    ...
    const cfg = await dbx.$queryRawUnsafe(`SELECT payroll_day, enabled FROM "CashFlowPayrollSetting" LIMIT 1`).catch(() => []);
    if (cfg[0] && cfg[0].enabled === false)
        return [];
    ...
    const pays = await dbx.$queryRawUnsafe(`SELECT employee_id, pay_type, hourly_rate, monthly_salary, employer_pct FROM "EmployeePay"`).catch(() => []);
    const payBy = new Map(pays.map((x) => [String(x.employee_id), x]));
    const emps = await dbx.$queryRawUnsafe(`SELECT id, role FROM "Employee"`).catch(() => []);
    const posRows = await dbx.$queryRawUnsafe(`SELECT position_name, hourly_rate FROM "WorkPosition" WHERE hourly_rate IS NOT NULL AND hourly_rate > 0`).catch(() => []);
    const norm = (x) => String(x || '').replace(/[\s"'׳״־\-\/\\|,.]+/g, '').toLowerCase();
    const posRate = new Map(posRows.map((x) => [norm(x.position_name), Number(x.hourly_rate)]));
    for (const e of emps) {
        const cur = payBy.get(String(e.id));
        if (cur && cur.hourly_rate != null && Number(cur.hourly_rate) > 0)
            continue;
        if (cur && cur.pay_type && cur.pay_type !== 'hourly')
            continue;
        const r = posRate.get(norm(e.role));
        if (r)
            payBy.set(String(e.id), { ...(cur || {}), pay_type: 'hourly', hourly_rate: r });
    }
    const start = new Date(today.getTime() - 45 * DAY_MS);
    const shifts = await dbx.$queryRawUnsafe(`SELECT date, assigned_staff FROM "WorkShift" WHERE date >= $1 AND date <= $2`, start, rangeEnd).catch(() => []);
    const toMin = (t) => { const m = String(t || '').match(/(\d{1,2}):(\d{2})/); return m ? (+m[1]) * 60 + (+m[2]) : null; };
    const hrsOf = (a, b) => { const x = toMin(a), y0 = toMin(b); if (x == null || y0 == null)
        return 0; let y = y0; if (y < x)
        y += 1440; return (y - x) / 60; };
    const gross = (h, r) => Math.min(h, 8) * r + Math.min(Math.max(h - 8, 0), 2) * r * 1.25 + Math.max(h - 10, 0) * r * 1.5;
    ...
    for (const sh of shifts) {
        ...
        for (const a of (Array.isArray(sh.assigned_staff) ? sh.assigned_staff : [])) {
            ...
            if (!rate || (pay?.pay_type && pay.pay_type !== 'hourly'))
                continue;
            let h = hrsOf(a.start_time, a.end_time);
            if (a.total_break_minutes)
                h = Math.max(0, h - Number(a.total_break_minutes) / 60);
            const mult = 1 + (Number(pay?.employer_pct) || 0) / 100;
            byMonth.set(mk, (byMonth.get(mk) || 0) + gross(h, rate) * mult);
        }
    }
    const monthlyFixed = pays
        .filter((x) => x.pay_type === 'monthly' && Number(x.monthly_salary) > 0)
        .reduce((n, x) => n + Number(x.monthly_salary) * (1 + (Number(x.employer_pct) || 0) / 100), 0);
    const out = [];
    for (const mk of byMonth.keys()) {
        ...
        if (payDate < today || payDate > rangeEnd)
            continue;
...
registerFn('getCashFlowOpening', async ({ user }) => {
    if (!isAdmin(user))
        throw new Error('forbidden');
    await requirePageAccess(user, 'CashFlow');
    const s = await prisma.cashFlowSetting.findFirst({ orderBy: { updatedAt: 'desc' } }).catch(() => null);
    return {
...
registerFn('setCashFlowOpening', async ({ body, user }) => {
    ...
    const existing = await prisma.cashFlowSetting.findFirst().catch(() => null);
    if (existing) {
        await prisma.cashFlowSetting.update({ where: { id: existing.id }, data: { opening_balance: bal, opening_date: date } });
    }
    else {
        await prisma.cashFlowSetting.create({ data: { opening_balance: bal, opening_date: date } });
    }
    return { ok: true };
});
...
registerFn('setDailyRevenue', async ({ body, user }) => {
    ...
    await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "CashFlowDailyRevenue" (
       ...
       "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)`).catch(() => { });
    await prisma.$executeRawUnsafe(`INSERT INTO "CashFlowDailyRevenue"("date","amount","note","updatedAt") VALUES ($1,$2,$3,NOW())
     ON CONFLICT ("date") DO UPDATE SET "amount"=EXCLUDED."amount","note"=EXCLUDED."note","updatedAt"=NOW()`, date, amount, b.note ? String(b.note).slice(0, 200) : null);
    return { ok: true, date, amount };
});
registerFn('getPayrollSetting', async ({ user }) => {
    if (!isAdmin(user))
        throw new Error('forbidden');
    const r = await prisma.$queryRawUnsafe(`SELECT payroll_day, enabled FROM "CashFlowPayrollSetting" LIMIT 1`).catch(() => []);
...
registerFn('setPayrollSetting', async ({ body, user }) => {
    ...
    await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "CashFlowPayrollSetting" (
       "id" TEXT PRIMARY KEY, "payroll_day" INTEGER NOT NULL DEFAULT 10,
       "enabled" BOOLEAN NOT NULL DEFAULT true,
       "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)`).catch(() => { });
    const ex = await prisma.$queryRawUnsafe(`SELECT id FROM "CashFlowPayrollSetting" LIMIT 1`).catch(() => []);
    if (ex[0]) {
        await prisma.$executeRawUnsafe(`UPDATE "CashFlowPayrollSetting" SET payroll_day=$2, enabled=$3, "updatedAt"=NOW() WHERE id=$1`, ex[0].id, day, enabled);
    }
    else {
        await prisma.$executeRawUnsafe(`INSERT INTO "CashFlowPayrollSetting"("id","payroll_day","enabled","updatedAt") VALUES ($1,$2,$3,NOW())`, randomUUID(), day, enabled);
    }
