...
function scoreLink(url, anchorText) {
    ...
    if (NEGATIVE.some(n => url.toLowerCase().includes(n)))
        return -1;
    let score = 0;
    ...
    if (PROVIDER_HINTS.some(h => host.includes(h.trim())))
        score += 3;
...
export function extractInvoiceLinks(html, text) {
    const candidates = new Map(); // url -> best score
    const consider = (rawUrl, anchorText) => {
    ...
    for (const u of (text || '').match(bareRe) || [])
        consider(u, '');
