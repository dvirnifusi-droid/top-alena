...
import { alertNewEventLead, alertBadCustomerFeedback, alertCriticalIncident, alertLargeReservation, alertCashDiscrepancy, } from './whatsappAlerts.js';
const db = prisma;
const handlers = {};
function on(model, event, h) {
    if (!handlers[model])
        handlers[model] = {};
    if (!handlers[model][event])
        handlers[model][event] = [];
    handlers[model][event].push(h);
}
export async function fireTriggers(model, event, row, prev) {
    ...
    for (const h of list) {
        try {
            ...
            console.error(`[trigger] ${model}/${event}:`, e?.message);
        }
    }
}
// ── helpers ──────────────────────────────────────────────────────────
const shortText = (s, n = 200) => String(s || '').slice(0, n);
const statusLabel = (s) => s === 'approved' ? 'אושרה ✓' :
    s === 'rejected' ? 'נדחתה ✕' :
        s === 'pending' ? 'ממתינה' :
            s === 'cancelled' ? 'בוטלה' :
                String(s || '-');
const shiftLabel = (t) => t === 'lunch' ? 'צהריים ☀️' :
    t === 'dinner' ? 'ערב 🌙' :
        String(t || '-');
const fmtMoney = (n) => (n == null || Number.isNaN(Number(n))) ? null : `₪${Number(n).toLocaleString('he-IL')}`;
const fmtDate = (v) => {
    if (!v)
        return '-';
    try {
...
const fmtTime = (v) => {
    ...
    try {
        const d = new Date(v);
        if (!isNaN(d.getTime()))
            return `${String(ilHourOf(d)).padStart(2, '0')}:${String(d.getUTCMinutes()).padStart(2, '0')}`;
    }
    catch { }
    return String(v).slice(11, 16);
};
const sevLabel = (s) => s === 'low' ? 'נמוכה 🟢' :
    s === 'medium' ? 'בינונית 🟡' :
        s === 'high' ? 'גבוהה 🟠' :
            s === 'critical' ? 'קריטית 🔴' :
                String(s || '-');
...
const link = (path) => `🔗 ${APP_BASE()}${path}`;
// ─────────────────── Pushover (to admins) ───────────────────
// 1. Incident — every field that's set goes into the push
on('Incident', 'created', async (row) => {
    const openN = await db.incident.count({ where: { OR: [{ status: { in: ['open', 'in_progress', 'reported', 'pending', 'new'] } }, { status: null }] } }).catch(() => null);
    const lines = [
        ...
        openN ? `🔴 סה"כ ${openN} תקלות פתוחות כרגע` : null,
        link('/Incidents'),
    ];
...
on('ChecklistExecution', 'updated', async (row, prev) => {
    ...
    const items = Array.isArray(row.results)
        ? row.results
    ...
    const failedSample = failed.slice(0, 3).map((f) => `• ${f.label || f.name || '-'}${f.notes ? ` (${shortText(f.notes, 50)})` : ''}`).join('\n');
    ...
    await pushoverToAdmins(failed.length === 0 ? `✅ צ'קליסט הושלם — תקין` : `⚠️ צ'קליסט הושלם עם בעיות`, join([
...
on('Reservation', 'created', async (row) => {
    // Same-day context: how busy is the book already for that date.
    const dayCtx = await (async () => {
        try {
            ...
            const active = (dayRows || []).filter((r) => r && r.status !== 'cancelled' && r.status !== 'no_show');
            const guests = active.reduce((sum, r) => sum + (Number(r.party_size) || 0), 0);
    ...
    await pushoverToAdmins(`📅 הזמנה חדשה — ${fmtDate(row.date)} ${row.time || ''}`.trim(), join(lines));
});
...
on('CustomerFeedback', 'created', async (row) => {
    ...
    await pushoverToAdmins(isNegative ? `🚨 משוב לקוח שלילי (${rating}/5)` : `⭐ משוב לקוח חיובי (${rating}/5)`, join(lines));
});
...
on('TipReport', 'updated', async (row, prev) => {
    ...
    // Per-employee distribution breakdown (defensive on shape — different flows
    // use different field names for name/amount/hours).
    const staffLines = [];
    if (Array.isArray(row.staff_details) && row.staff_details.length) {
        staffLines.push('— חלוקה לפי עובד:');
        for (const s of row.staff_details.slice(0, 8)) {
            if (!s)
                continue;
            const name = s.employee_name || s.name || s.full_name || '-';
            const amount = s.tip_amount ?? s.amount ?? s.total ?? s.final_amount;
            const hours = s.hours;
            const money = fmtMoney(amount);
            staffLines.push(`• ${name}${money ? ` — ${money}` : ''}${hours != null ? ` (${hours} שע')` : ''}`);
        }
    }
    const lines = [
        ...
        row.notes ? `📝 ${shortText(row.notes, 100)}` : null,
        ...staffLines,
        '↩️ פרטים מלאים ב-/Tips',
        link('/Tips'),
    ];
    await pushoverToAdmins(`💰 טיפים ננעלו — ${shiftLabel(row.shift_type)} ${fmtDate(row.date || row.shift_date)}`, join(lines));
});
...
on('EmployeeAvailability', 'created', async (row) => {
    const typeLabel = row.availability_type === 'available' ? '✅ פנוי/ה' :
    ...
    const shiftPref = row.shift_preference === 'lunch' ? 'צהריים' :
...
on('LeaveRequest', 'created', async (row) => {
    ...
    const lines = [
        ...
        `↩️ השב *"אשר חופשה של ${row.employee_name || '-'}"* או *"דחה"*`,
        link('/LeaveRequests'),
    ];
...
on('Interview', 'created', async (row) => {
    const typeLabel = row.type === 'menu_exam' ? 'בחינת תפריט' :
    ...
    const lines = [
        ...
        '↩️ נקבע אוטומטית · פרטים ב-/Recruitment',
        link('/Recruitment'),
    ];
...
on('JobCandidate', 'created', async (row) => {
    const isPlaceholder = (!row.full_name || row.full_name === 'מועמד בתהליך' || row.full_name === 'מועמד אנונימי') &&
        !row.phone;
    if (isPlaceholder)
        return;
    const weekN = await db.jobCandidate.count({ where: { createdAt: { gte: new Date(Date.now() - 7 * 86400000) } } }).catch(() => null);
    const scoreEmoji = row.score == null ? '❓' :
        row.score >= 80 ? '🟢' :
            row.score >= 50 ? '🟡' : '🔴';
    const lines = [
        ...
        weekN != null ? `📈 מועמדים השבוע: ${weekN}` : null,
        link('/Recruitment'),
    ];
...
on('EventBooking', 'updated', async (row, prev) => {
    ...
    if (approvalChanged && row.approval_status === 'approved') {
        await pushoverEventsOwners(row.short_notice ? `⚡ אירוע same-day נסגר!` : `🎉 אירוע נסגר — אישר ע"י מנהל`, join([
            ...baseLines,
            row.payment_status ? `💳 סטטוס תשלום: ${row.payment_status}` : null,
            row.approval_notes ? `📝 הערות: ${shortText(row.approval_notes, 100)}` : null,
            link('/EventBookings'),
        ]));
        return;
    }
    if (approvalChanged && row.approval_status === 'rejected') {
        await pushoverEventsOwners(`❌ אירוע נדחה ע"י מנהל`, join([...baseLines, row.approval_notes ? `📝 סיבה: ${shortText(row.approval_notes, 120)}` : null, link('/EventBookings')]));
        return;
    }
    ...
    if (statusChanged && (row.status === 'confirmed' || row.status === 'pending_manager_callback')) {
        await pushoverEventsOwners(row.status === 'pending_manager_callback'
            ? `🎯 אירוע נסגר — התקשר ללקוח`
            : `🎉 אירוע אושר`, join([...baseLines, link('/EventBookings')]));
    }
});
...
on('LeaveRequest', 'updated', async (row, prev) => {
    if (prev?.status === row.status)
        return;
    await notifyEmployee(row.employee_id, `🌴 בקשת החופשה ${statusLabel(row.status)}`, `${row.start_date || ''}${row.end_date ? `–${row.end_date}` : ''}`, '/LeaveRequests');
});
...
on('ShiftSwapRequest', 'updated', async (row, prev) => {
    if (prev?.status === row.status)
        return;
    await notifyEmployee(row.requester_id, `🔄 בקשת ההחלפה ${statusLabel(row.status)}`, `המשמרת בתאריך ${row.original_date || row.shift_date || '-'}`, '/EmployeeHome');
});
