...
const dbx = () => prisma;
const ils = (n) => `₪${Math.round(Math.abs(n)).toLocaleString()}`;
const SOON_DAYS = 5;
export async function runOverduePaymentAlerts(opts = {}) {
    ...
    const rows = await dbx().$queryRawUnsafe(`SELECT i.id, i.supplier_id, i.invoice_date, i.total_amount,
    ...
    const byName = (list) => {
        const m = new Map();
        for (const x of list)
            m.set(x.name, (m.get(x.name) || 0) + x.amount);
        return [...m.entries()].sort((a, b) => b[1] - a[1]);
    };
    const lines = ['💰 תשלומים לספקים'];
    if (late.length) {
        const t = late.reduce((s, x) => s + x.amount, 0);
        lines.push('', `🔴 באיחור — ${late.length} חשבוניות · ${ils(t)}`);
        for (const [name, amt] of byName(late).slice(0, 8))
            lines.push(`   • ${name} — ${ils(amt)}`);
        if (byName(late).length > 8)
            lines.push(`   ועוד ${byName(late).length - 8} ספקים`);
    }
    if (soon.length) {
        const t = soon.reduce((s, x) => s + x.amount, 0);
        lines.push('', `🟡 לתשלום ב-${SOON_DAYS} הימים הקרובים — ${soon.length} חשבוניות · ${ils(t)}`);
        for (const [name, amt] of byName(soon).slice(0, 8))
            lines.push(`   • ${name} — ${ils(amt)}`);
    }
    lines.push('', 'לפרטים: topalena.com/CashFlow');
    const text = lines.join('\n');
    if (opts.dryRun)
        return { ok: true, late: late.length, soon: soon.length, sent: false, preview: text };
...
//# sourceMappingURL=overdueAlerts.js.map