...
function wallToUtc(ymd, hhmm, addDays = 0) {
...
function fmtRange(since, now) {
    const f = (d) => new Intl.DateTimeFormat('he-IL', { timeZone: TZ, day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).format(d);
    return `${f(since)} → ${f(now)}`;
}
// Build the report for shifts that closed in (since, now].
export async function buildDailyHoursReport(since, now = new Date()) {
    ...
    const dateSet = new Set([sinceYMD, nowYMD]);
    // ── Actual hours — ShiftTracking rows that CLOSED in the window ──
    const tracks = await db.shiftTracking.findMany({
    ...
    const dayStart = new Date(`${sinceYMD}T00:00:00.000Z`);
    ...
    for (const t of tracks) {
        ...
        if (t.employee_id) {
            ...
            if (!prev || hrs > (Number(prev.total_hours ?? prev.effective_hours) || 0))
                actualByEmp.set(t.employee_id, t);
        }
        const nm = normName(t.employee_name);
        if (nm) {
            const prevN = actualByName.get(nm);
            if (!prevN || hrs > (Number(prevN.total_hours ?? prevN.effective_hours) || 0))
                actualByName.set(nm, t);
        }
    }
    // Names + roles for everyone involved.
    const involvedIds = new Set([...actualByEmp.keys()]);
    ...
    for (const id of involvedIds) {
        ...
        if (!track) {
            ...
            if (nm && actualByName.has(nm))
                track = actualByName.get(nm);
        }
        const name = track?.employee_name || emp?.full_name || 'עובד';
        const role = roster?.role || emp?.role || '';
        const flags = [];
        let hours = null;
        let shift_type = roster?.shift_type || track?.shift_type || null;
    ...
    // ── WhatsApp text ──
    const roleTag = (r) => (r ? ` _(${r})_` : '');
    const lines = [`🕐 *דוח שעות יומי*`, `_${fmtRange(since, now)}_`, ''];
    if (!rows.length) {
        ...
        for (const r of rows) {
            if (r.hours == null)
                lines.push(`• ${r.name}${roleTag(r.role)} — ⚠️ בסידור, לא נמצאה החתמת שעון`);
            else
                lines.push(`• ${r.name}${roleTag(r.role)} — *${r.hours.toFixed(2)}* שע'${r.flags.includes('long') ? ' ⚠️ משמרת ארוכה' : ''}`);
        }
...
async function adminEmails() {
    ...
    const set = new Set(users.map(u => u.email).filter(Boolean));
...
export async function sendDailyHoursReport(opts = {}) {
    const now = new Date();
    const profile = await db.restaurantProfile.findFirst().catch(() => null);
    const since = profile?.last_daily_hours_report_at
    ...
    try {
        ...
        console.warn('[daily-hours] push failed', e?.message);
    }
    ...
    for (const to of emails) {
        try {
            await sendEmail({ to, subject: `🕐 דוח שעות יומי — ${now.toLocaleDateString('he-IL', { timeZone: TZ })}`, html: report.html, text: report.text });
            emailSent++;
        }
        catch (e) {
            console.warn('[daily-hours] email failed', { to, err: e?.message });
        }
    }
    ...
    if (profile?.id) {
        ...
        }).catch((e) => console.warn('[daily-hours] cursor update failed', e?.message));
    }
...
export async function checkDailyHoursReportSchedule() {
    try {
        ...
        if (minutes < slotMin || minutes >= slotMin + 20)
            return;
        ...
        if (last && now.getTime() - last < 12 * 3600 * 1000)
            return;
...
//# sourceMappingURL=dailyHoursReport.js.map