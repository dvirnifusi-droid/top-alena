...
export async function ensureClubTables() {
    ...
    await dbx().$executeRawUnsafe(`CREATE UNIQUE INDEX IF NOT EXISTS "CustomerBenefit_code_key" ON "CustomerBenefit"(code) WHERE code IS NOT NULL`).catch(() => { });
    // Credit a game session only once, however many times the finish call arrives.
    await dbx().$executeRawUnsafe(`ALTER TABLE "QueueGameSession" ADD COLUMN IF NOT EXISTS coins_awarded INT DEFAULT 0`).catch(() => { });
    ...
    await dbx().$executeRawUnsafe(`ALTER TABLE "QueueGameSession" ADD COLUMN IF NOT EXISTS customer_id TEXT`).catch(() => { });
    await dbx().$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "QueueGameSession_customer_idx" ON "QueueGameSession"(customer_id)`).catch(() => { });
}
...
export async function grantBenefit(opts) {
    if (!opts.customerId || !opts.description)
        return null;
    await ensureClubTables();
    if (opts.source) {
        const existing = await dbx().$queryRawUnsafe(`SELECT * FROM "CustomerBenefit"
        WHERE customer_id = $1 AND source = $2 AND status = 'active'
          AND (expiry_date IS NULL OR expiry_date >= NOW())
        LIMIT 1`, opts.customerId, opts.source).catch(() => []);
        if (existing?.length)
            return rowToBenefit(existing[0]);
    }
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        ...
        const code = makeCode();
        try {
            await dbx().$executeRawUnsafe(`INSERT INTO "CustomerBenefit"
           (id, customer_id, description, type, status, code, source, expiry_date, "createdAt", "updatedAt")
         VALUES ($1, $2, $3, $4, 'active', $5, $6, NOW() + ($7 || ' days')::interval, NOW(), NOW())`, id, opts.customerId, opts.description, opts.type_ || 'gift', code, opts.source || null, String(days));
            const rows = await dbx().$queryRawUnsafe(`SELECT * FROM "CustomerBenefit" WHERE id = $1`, id);
...
export async function maybeGrantPunchCard(customerId, visitCount) {
    if (!customerId || !visitCount)
        return null;
    const cfg = await getClubConfig().catch(() => null);
    if (!cfg?.punchcard_enabled)
        return null;
    const t = Number(cfg.punchcard_threshold) || 10;
    if (t < 1 || visitCount % t !== 0)
        return null;
...
export async function listBenefits(customerId) {
    ...
    const rows = await dbx().$queryRawUnsafe(`SELECT * FROM "CustomerBenefit"
      ...
      LIMIT 50`, customerId).catch(() => []);
    const all = (rows || []).map(rowToBenefit);
    const now = Date.now();
    const alive = (b) => b.status === 'active' && (!b.expiry_date || new Date(`${b.expiry_date}T23:59:59`).getTime() >= now);
    return { active: all.filter(alive), used: all.filter((b) => !alive(b)).slice(0, 10) };
}
export async function findBenefitByCode(code) {
    if (!code)
        return null;
    await ensureClubTables();
    const rows = await dbx().$queryRawUnsafe(`SELECT b.*, c.name AS customer_name, c.phone AS customer_phone
       FROM "CustomerBenefit" b
       LEFT JOIN "Customer" c ON c.id = b.customer_id
      WHERE UPPER(b.code) = UPPER($1)
      LIMIT 1`, String(code).trim()).catch(() => []);
    if (!rows?.length)
        return null;
 ...
 */
export async function redeemBenefit(code, by) {
    const b = await findBenefitByCode(code);
    if (!b)
        return { ok: false, reason: 'not_found' };
    if (b.status !== 'active' || b.redeemed_at)
    ...
    const n = await dbx().$executeRawUnsafe(`UPDATE "CustomerBenefit"
        SET status = 'redeemed', redeemed_at = NOW(), redeemed_by = $2, "updatedAt" = NOW()
      WHERE id = $1 AND status = 'active' AND redeemed_at IS NULL`, b.id, by || 'צוות');
    if (!n)
        return { ok: false, reason: 'already_used', benefit: b };
 ...
 */
export async function tonightBoard() {
    await ensureClubTables();
    const rows = await dbx().$queryRawUnsafe(`SELECT e.id AS entry_id,
...
//# sourceMappingURL=clubCore.js.map