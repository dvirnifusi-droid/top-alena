...
const q = async (sql, ...params) => {
    try {
        ...
        console.warn('[forecast]', String(e?.message || '').slice(0, 120));
        return [];
    }
};
...
export async function getDemandHistory(days = 60) {
    const rows = await q(`SELECT date, diners, orders_count, net_total FROM "BeecommHistoricalDay"
...
async function weekdayRevenueAverages() {
    ...
    for (const r of rows) {
        ...
        if (!sums[wd])
            sums[wd] = { t: 0, n: 0 };
        sums[wd].t += Number(r.net_total);
        sums[wd].n++;
    }
    const out = {};
    for (const [wd, s] of Object.entries(sums))
        out[Number(wd)] = round(s.t / Math.max(1, s.n));
    return out;
}
...
export async function forecastCashFlow() {
    ...
    for (const inv of openInv) {
        ...
        if (inv.due_date) {
            ...
            if (t >= start.getTime() && t < windowEnd.getTime()) {
                (invByDay[dd] ||= []).push(inv);
            }
            else if (t >= windowEnd.getTime())
                scheduledBeyond.push({ date: dd, supplier: inv.supplier, amount: round(amt) });
            // due in the past & still unpaid → treat as due today (overdue)
            else if (t < start.getTime()) {
                (invByDay[todayStr] ||= []).push({ ...inv, _overdue: true });
            }
        }
        else {
            unpaidNoDate += amt;
    ...
    for (let i = 0; i < 7; i++) {
        ...
        for (const p of planned) {
            if (String(p.date).slice(0, 10) !== ds)
                continue;
            ...
            if (p.type === 'income')
                ...
                expOut += amt;
                outItems.push({ name: p.source || p.description || p.category, amount: round(amt) });
            }
        }
        for (const rc of recurring) {
            if (Number(rc.day_of_month) === dom) {
                expOut += Number(rc.amount || 0);
                outItems.push({ name: `${rc.name} (קבוע)`, amount: round(Number(rc.amount || 0)) });
            }
        }
    ...
    const beyondTotal = scheduledBeyond.reduce((a, r) => a + Number(r.amount || 0), 0);
    const alerts = [];
    for (const d of days)
        if (d.net < 0)
            alerts.push(`⚠️ ${d.weekday} ${d.date}: צפי שלילי של ₪${Math.abs(d.net).toLocaleString()}`);
    if (unpaidNoDateCount > 0)
        alerts.push(`📄 ${unpaidNoDateCount} חשבוניות לא משולמות בסך ₪${round(unpaidNoDate).toLocaleString()} עדיין בלי תאריך תשלום — קבע להן "ישולם בתאריך" כדי שייכנסו לתזרים`);
    return {
        days,
...
export async function predictNoShows() {
    ...
    for (const h of history) {
        const p = digits(h.customer_phone);
        if (!p)
            continue;
        if (!byPhone[p])
            byPhone[p] = { total: 0, bad: 0, cancelled: 0 };
        byPhone[p].total++;
        if (String(h.status) === 'no_show')
            byPhone[p].bad++;
        else if (String(h.status) === 'cancelled')
            byPhone[p].cancelled++;
    }
    const scored = upcoming.map((r) => {
        const p = digits(r.customer_phone);
        const h = p ? byPhone[p] : undefined;
        let risk = 10;
        const reasons = [];
        if (h?.bad) {
            risk += Math.min(45, h.bad * 30);
            reasons.push(`${h.bad} אי-הגעות בעבר`);
        }
        if (h && h.cancelled >= 2) {
            risk += 15;
            reasons.push(`${h.cancelled} ביטולים בעבר`);
        }
        if (!h) {
            const wd = weekdayOf(String(r.date).slice(0, 10));
            if (wd === 5 || wd === 6) {
                risk += 12;
                reasons.push('לקוח חדש בסופ"ש');
            }
            else
                reasons.push('לקוח ללא היסטוריה');
            risk += 5;
        }
        if (Number(r.party_size) >= 6) {
            risk += 12;
            reasons.push(`שולחן גדול (${r.party_size})`);
        }
        if (String(r.status || 'pending') === 'pending') {
            risk += 10;
            reasons.push('טרם אושרה');
        }
        return {
...
export async function forecastLaborCost() {
    ...
    const payByEmp = {};
    for (const p of pays)
        payByEmp[String(p.employee_id)] = p;
    ...
    const positions = await q(`SELECT position_name, hourly_rate FROM "WorkPosition" WHERE hourly_rate IS NOT NULL AND hourly_rate > 0`);
    const normP = (x) => String(x || '').replace(/[\s"'׳״־\-/\|,.]+/g, '').toLowerCase();
    const posRate = {};
    for (const p of positions)
        posRate[normP(p.position_name)] = Number(p.hourly_rate);
    for (const e of emps) {
    ...
    const nameToId = {};
    for (const e of emps)
        nameToId[String(e.full_name || '').trim().toLowerCase()] = String(e.id);
    const shiftHours = (ws) => {
        const parse = (t) => { const m = String(t || '').match(/^(\d{2}):(\d{2})/); return m ? Number(m[1]) + Number(m[2]) / 60 : null; };
        const a = parse(ws.start_time), b = parse(ws.end_time);
        if (a == null || b == null)
            return 6; // sensible default shift length
    ...
    for (const ws of shifts) {
        ...
        if (!byDate[ds])
            byDate[ds] = { cost: 0, hours: 0, staff: 0, missing: new Set() };
        ...
        for (const a of staff) {
            ...
            if (!pay) {
                byDate[ds].missing.add(String(a?.name || a?.employee_name || id || '?'));
                continue;
            }
            let hourly = Number(pay.hourly_rate || 0);
            if (!hourly && pay.pay_type === 'monthly' && pay.monthly_salary)
                hourly = Number(pay.monthly_salary) / 182;
...
export async function detectSupplierPriceDrift() {
    const rows = await q(`SELECT ii.product_name, ii.unit_price, ii.quantity, i.invoice_date, s.name AS supplier
     FROM "InvoiceItem" ii
...
export async function forecastEventDemand() {
    ...
    const monthOf = (d) => String(new Date(d).toISOString()).slice(0, 7);
    const months = {};
    const bookedLeadIds = new Set(bookings.map((b) => String(b.lead_id || '')));
    for (const l of leads) {
        const m = monthOf(l.createdAt);
        if (!months[m])
            months[m] = { leads: 0, booked: 0 };
        months[m].leads++;
        if (bookedLeadIds.has(String(l.id)) || ['booked', 'won', 'closed'].includes(String(l.status || '')))
            months[m].booked++;
    }
    ...
    const todayStr = ilTodayStr();
    const upcoming = bookings.filter((b) => String(b.event_date) >= todayStr);
    const upcomingRevenue = upcoming.reduce((a, b) => a + Number(b.total_ils || 0), 0);
    ...
    return {
        monthly,
        upcoming: { count: upcoming.length, guests: upcoming.reduce((a, b) => a + Number(b.guest_count || 0), 0), revenue: round(upcomingRevenue) },
        projection: {
...
//# sourceMappingURL=businessForecasts.js.map