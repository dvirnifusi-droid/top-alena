 ...
 */
export async function diagnoseGoogleWallet() {
    const steps = [];
    ...
    const reason = (d) => d?.error?.message || JSON.stringify(d || {}).slice(0, 200);
    const issuer = await call('GET', `/issuer/${issuerId}`);
    if (issuer.status === 200) {
        push('הרשאה על המנפיק', true, `מזוהה בשם "${issuer.data?.name || ''}"`);
    }
    else {
        push('הרשאה על המנפיק', false, `${reason(issuer.data)} — בדרך כלל חסר לצרף את חשבון השירות תחת Users בקונסולת Wallet`);
        return { ok: false, steps };
    }
    const classId = `${issuerId}.club`;
