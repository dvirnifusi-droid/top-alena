...
export function reconcile(txs, invoices) {
    const used = new Set(); // invoice ids already attributed
    const matched = new Set(); // bank tx ids already attributed
    const matches = [];
    ...
    const exactPass = () => {
        for (const tx of txs) {
            ...
            if (cands.length > 1) {
                ...
                if (cands[0].supplier_id !== cands[1].supplier_id && g1 - g0 <= 2)
                    continue;
            }
            const inv = cands[0];
    ...
    const batchPass = () => {
        for (const tx of txs) {
            ...
            for (const i of open()) {
                ...
                if (g < -EARLY || g > LATE)
                    continue;
                if (i.amount > target + AGORA)
                    continue; // cannot be part of a smaller sum
                if (!bySupplier.has(i.supplier_id))
                    bySupplier.set(i.supplier_id, []);
...
//# sourceMappingURL=bankMatch.js.map