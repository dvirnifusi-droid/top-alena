...
const sql = (q, ...args) => prisma.$executeRawUnsafe(q, ...args);
const query = (q, ...args) => prisma.$queryRawUnsafe(q, ...args);
...
function summarizeState(data, tenant) {
    ...
    if (c.checklists)
        parts.push(`צ׳קליסטים: ${c.checklists}`);
    if (c.suppliers)
        parts.push(`ספקים: ${c.suppliers}`);
    if (c.customers)
        parts.push(`לקוחות במועדון: ${c.customers}`);
    if (c.knowledge)
        parts.push(`מסמכי ידע: ${c.knowledge}`);
    if (c.invoice_email)
        parts.push(`מייל חשבוניות: הוגדר`);
    return parts.join('\n');
}
...
function pickReply(brainResult, deterministic) {
...
function fallbackReply(nm) {
    const q = FALLBACK_Q[nm.key];
    return q ? (nm.key === 'finish' ? q : `בוא נמשיך 🙂 ${q}`) : 'אפשר לכתוב לי את זה שוב? 🙏';
}
function nextMissing(data) {
    const c = data._counts || {};
    if (!data.name)
        return { key: 'name', offer: 'שם המסעדה' };
    if (!data.address)
        return { key: 'address', offer: 'כתובת מדויקת (רחוב, מספר, עיר)' };
    if (!data.opening_hours)
        return { key: 'hours', offer: 'שעות פתיחה' };
    if (!data.cuisine)
        return { key: 'cuisine', offer: 'סוג המטבח (בשורה אחת)' };
    const done = new Set(Array.isArray(data._done_modules) ? data._done_modules : []);
...
async function onboardingBrain(tenant, history, message, data) {
    ...
    const prompt = `אתה "העוזר החכם" של TOP APOLLO — מקים מסעדה בשם "${data.name || tenant.restaurant_name}" עבור ${tenant.owner_name || 'הבעלים'}.\n` +
...
async function runBrainAction(tenant, action, data) {
    ...
    try {
        ...
        if (action === 'suggest_roles') {
            const roles = await aiSuggestRoles(tenant, data);
            data._counts.roles = (data._counts.roles || 0) + roles.length;
...
async function applyExtraction(tenant, result, data) {
    ...
    if (data.name || data.address) {
        await persistCoreData(tenant, data).catch((e) => console.warn('[onboarding] core persist:', e?.message));
        data._counts = data._counts || {};
        if (Number(data.tables_count) > 0)
            data._counts.tables = Number(data.tables_count);
    }
    data._counts = data._counts || {};
    ...
    if (kind && text) {
        try {
            if (kind === 'employees')
                data._counts.employees = (data._counts.employees || 0) + await insertEmployeesFromText(tenant, text);
            else if (kind === 'suppliers')
                data._counts.suppliers = (data._counts.suppliers || 0) + await insertSuppliersFromText(tenant, text);
            else if (kind === 'roles')
                data._counts.roles = (data._counts.roles || 0) + await insertRolesFromText(tenant, text);
            else if (kind === 'interview_slots')
                data._counts.slots = (data._counts.slots || 0) + await insertInterviewSlots(tenant, text);
...
async function classifyFile(tenant, mediaUrl) {
    const { invokeLLM } = await import('./llm.js');
    const c = await invokeLLM({
        prompt: `הקובץ המצורף שייך למסעדה. סווג אותו לאחת מהקטגוריות:\n` +
    ...
    return {
        ...
        sample: Array.isArray(c?.sample) ? c.sample.map((s) => String(s)).slice(0, 3) : [],
    };
}
// Runs the right extractor for a confirmed kind and updates counts.
async function importByKind(tenant, kind, mediaUrl, data) {
    data._counts = data._counts || {};
    let count = 0;
    try {
        if (kind === 'menu') {
            count = await extractAndInsertMenu(tenant, mediaUrl);
            data._counts.menu = (data._counts.menu || 0) + count;
        }
        else if (kind === 'work_schedule') {
            count = await extractAndInsertRolesFromFile(tenant, mediaUrl);
            data._counts.roles = (data._counts.roles || 0) + count;
        }
        else if (kind === 'employee_list') {
            count = await extractAndInsertEmployeesFromFile(tenant, mediaUrl);
            data._counts.employees = (data._counts.employees || 0) + count;
        }
        else if (kind === 'checklist') {
            count = await extractAndInsertChecklists(tenant, mediaUrl, data);
            data._counts.checklists = (data._counts.checklists || 0) + count;
        }
        else if (kind === 'customer_list') {
            count = await extractAndInsertCustomers(tenant, mediaUrl);
            data._counts.customers = (data._counts.customers || 0) + count;
        }
        else if (kind === 'seating_map') {
            count = await extractAndInsertSeating(tenant, mediaUrl);
            data._counts.seating = (data._counts.seating || 0) + count;
        }
        else {
            count = await extractAndInsertKnowledge(tenant, mediaUrl, 'כללי');
            data._counts.knowledge = (data._counts.knowledge || 0) + count;
        }
    }
    catch (e) {
...
// === Public API ============================================================
export async function startOnboarding(tenantId) {
    await ensureOnboardingRow(tenantId);
    const tenant = await getTenant(tenantId);
    if (!tenant)
        return;
    const first = `שלום ${tenant.owner_name || ''}! 🌿 אני העוזר החכם של TOP APOLLO, ואני אקים לך את המסעדה תוך כמה דקות — ` +
    ...
    const data = { _history: [{ role: 'assistant', content: first }] };
...
export async function tryHandleOnboardingMessage(fromPhone, body) {
    ...
    if (data._pending_website?.url) {
        ...
        if (CONFIRM_YES_RE.test(body.trim())) {
            ...
            for (const k of ['name', 'address', 'phone', 'opening_hours', 'cuisine', 'description']) {
                if (b[k] != null && String(b[k]).trim() && !data[k])
                    data[k] = String(b[k]).trim();
            }
            ...
            history.push({ role: 'user', content: body });
            const labels = { name: 'שם', address: 'כתובת', opening_hours: 'שעות', cuisine: 'מטבח' };
    ...
    if (data._pending_seating?.tables) {
        ...
        if (CONFIRM_YES_RE.test(body.trim()) || corrected === 'seating_map') {
            ...
            if (!data._done_modules.includes('seating'))
                data._done_modules.push('seating');
    ...
    if (urlHit) {
        ...
        void (async () => {
            ...
            const d = fresh?.collected_data || data;
            const h = Array.isArray(d._history) ? d._history : history;
            ...
            if (ext && (ext.dishes.length || hasBiz)) {
                ...
                const sample = ext.dishes.slice(0, 4).map((x) => String(x?.name || '')).filter(Boolean).join('* · *');
                const msg = `מצאתי באתר 🌐\n${detail || '(לא זוהו פרטי עסק)'}\n\n` +
    ...
    if (!menuAlreadyDone && !data._menu_parsing && looksLikeMenu(body)) {
        ...
        void (async () => {
            ...
            if (n > 0) {
                ...
                if (!d._done_modules.includes('menu'))
                    d._done_modules.push('menu');
            }
            const synthetic = n > 0
    ...
    if (!checklistsDone && !data._checklist_text_parsing && looksLikeChecklist(body)) {
        ...
        void (async () => {
            ...
            if (n > 0) {
                d._done_modules = Array.isArray(d._done_modules) ? d._done_modules : [];
                if (!d._done_modules.includes('checklists'))
                    d._done_modules.push('checklists');
            }
            const names = lists.map((l) => String(l?.title || '')).filter(Boolean).slice(0, 5).join(', ');
            const synthetic = n > 0
    ...
    if (data._pending_checklist_qa) {
        ...
        void (async () => {
            const n = await aiSuggestChecklists(tenant, data, ctx).catch((e) => { console.warn('[onboarding] checklist gen:', e?.message); return 0; });
            const fresh = await findActiveOnboarding(fromPhone).catch(() => null);
            const d = fresh?.collected_data || data;
            const h = Array.isArray(d._history) ? d._history : history;
            d._counts = d._counts || {};
    ...
    if (SKIP_RE.test(body) && data._asking && data._asking !== 'name' && data._asking !== 'address' && data._asking !== 'hours' && data._asking !== 'cuisine') {
        data._done_modules = Array.isArray(data._done_modules) ? data._done_modules : [];
        if (!data._done_modules.includes(data._asking))
            data._done_modules.push(data._asking);
    }
    const result = await onboardingBrain(tenant, history, body, data)
        .catch((e) => { console.warn('[onboarding] brain:', e?.message); return { reply: 'סליחה, רגע קטן... אפשר לכתוב שוב? 🙏' }; });
    await applyExtraction(tenant, result, data).catch((e) => console.warn('[onboarding] apply:', e?.message));
    ...
    const TOPIC_ACTION = { roles: 'suggest_roles', checklists: 'suggest_checklists', training: 'suggest_training' };
    ...
    if (result.asking)
        data._asking = String(result.asking);
    history.push({ role: 'assistant', content: reply });
    data._history = history.slice(-24);
    ...
    if (finished) {
        ...
        data._counts = data._counts || {};
        if (summary?.tables)
            data._counts.tables = summary.tables;
        ...
        await setPhase(state.tenant_id, 'done', data).catch((e) => console.warn('[onboarding] setPhase done:', e?.message));
        await sendWhatsApp(fromPhone, buildDoneMessage(tenant, data)).catch((e) => console.warn('[onboarding] send done:', e?.message));
    }
    else {
        await setPhase(state.tenant_id, 'active', data).catch((e) => console.warn('[onboarding] setPhase active:', e?.message));
        await sendWhatsApp(fromPhone, reply).catch((e) => console.warn('[onboarding] send reply:', e?.message));
    }
    return true;
}
...
export async function tryHandleOnboardingMedia(fromPhone, mediaUrl, _contentType) {
    const state = await findActiveOnboarding(fromPhone);
    if (!state)
        return false;
    const tenant = await getTenant(state.tenant_id);
    ...
    void (async () => {
        ...
        if (kind === 'seating_map') {
            ...
            if (tables.length) {
                ...
                const msg = `🪑 קיבלתי את הסקיצה ובניתי ממנה טיוטת מפה:\n*${seatingSummary(tables)}*\n\n` +
        ...
        const msg = `📋 קיבלתי את הקובץ. ${hedge}נראה לי ש*זה ${label}*.${sampleTxt}\n\n` +
...
async function persistCoreData(tenant, data) {
    ...
    try {
        ...
        const existing = await query(`SELECT id FROM "${schema}"."RestaurantProfile" LIMIT 1`);
        const hoursJson = JSON.stringify(String(data.opening_hours || ''));
        if (existing.length) {
            await sql(`UPDATE "${schema}"."RestaurantProfile" SET
         ...
         WHERE id = $7`, finalName, data.address || null, data.phone || null, data.opening_hours ? hoursJson : null, data.cuisine || null, data.description || null, existing[0].id);
        }
        else {
            await sql(`INSERT INTO "${schema}"."RestaurantProfile"
           ("id", "restaurant_name", "address", "phone", "opening_hours", "cuisine_style", "description", "createdAt", "updatedAt")
         VALUES ($1, $2, $3, $4, $5::jsonb, $6, $7, NOW(), NOW())`, await uuid(), finalName, data.address || null, data.phone || null, data.opening_hours ? hoursJson : JSON.stringify(''), data.cuisine || null, data.description || null);
        }
    ...
    if (tableCount > 0 && tableCount <= 200) {
        try {
            const existing = await query(`SELECT COUNT(*)::int AS n FROM "${schema}"."SeatingLayout"`);
            if ((existing[0]?.n || 0) === 0) {
                ...
                await sql(`INSERT INTO "${schema}"."SeatingLayout" ("id", "layout_name", "tables", "createdAt", "updatedAt")
           VALUES ($1, 'מפה ראשית', $2::jsonb, NOW(), NOW())`, await uuid(), JSON.stringify(tables));
...
async function llmExtract(mediaUrl, prompt, schemaProps, listKey, tenantSlug) {
    const { invokeLLM } = await import('./llm.js');
    const result = await invokeLLM({
        prompt,
        fileUrls: [mediaUrl],
...
async function extractAndInsertMenu(tenant, mediaUrl) {
    // listKey `dishes`, NOT `items` (Gemini keyword collision → empty).
    const items = await llmExtract(mediaUrl, `הקובץ הוא תפריט מסעדה. חלץ את כל המנות: name, category (ראשונות/עיקריות/שתייה... או "כללי"), price (מספר בלבד), description אם יש. אל תמציא.`, { name: { type: 'string' }, category: { type: 'string' }, price: { type: 'number' }, description: { type: 'string' } }, 'dishes', tenant.slug);
...
async function extractDishesFromText(tenant, text) {
    const { invokeLLM } = await import('./llm.js');
    const r = await invokeLLM({
        prompt: `הטקסט הבא הוא תפריט מסעדה שהבעלים הדביק בצ׳אט. חלץ את *כל* המנות — אל תשמיט אף אחת ואל תמציא.\n` +
...
async function extractChecklistsFromText(tenant, text) {
    const { invokeLLM } = await import('./llm.js');
    const r = await invokeLLM({
        prompt: `הטקסט הבא הוא רשימות תפעוליות של מסעדה (צ׳קליסטים / פתיחה / סגירה / רשימות הכנה) שהבעלים הדביק — לפעמים מקובץ Word/Excel.\n` +
...
async function insertDishes(tenant, dishes) {
    ...
    for (const it of (Array.isArray(dishes) ? dishes : []).slice(0, 400)) {
        ...
        if (!name)
            continue;
        await sql(`INSERT INTO "${schema}"."MenuItem" ("id", "name", "category", "description", "price", "available", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, $4, $5, true, NOW(), NOW())`, await uuid(), name.slice(0, 200), String(it?.category || 'כללי').slice(0, 80), it?.description ? String(it.description).slice(0, 500) : null, Number(it?.price) || 0).then(() => n++).catch((e) => console.warn('[onboarding] menu insert:', e?.message));
    }
    return n;
}
// === Website import ========================================================
...
function extractJsonLd(html) {
    ...
    while ((m = re.exec(html)) && out.length < 4)
        out.push(m[1].trim().slice(0, 4000));
    return out.length ? `\n\n[JSON-LD]\n${out.join('\n')}` : '';
}
function findRelevantLinks(html, base) {
    let origin;
    try {
        origin = new URL(base);
    }
    ...
    while ((m = re.exec(html)) && links.length < 6) {
        ...
        try {
            ...
            if (abs.origin !== origin.origin)
                continue; // same-site only
            const s = abs.toString().split('#')[0];
            if (s !== base && !links.includes(s))
                links.push(s);
        }
...
async function extractFromWebsite(tenant, url) {
    const text = await fetchWebsiteText(url);
    if (!text || text.length < 40)
        return null;
    const { invokeLLM } = await import('./llm.js');
    const result = await invokeLLM({
        prompt: `להלן טקסט שחולץ מאתר אינטרנט של מסעדה/עסק אוכל. חלץ ממנו רק מה שבאמת מופיע — אל תמציא כלום.\n` +
...
async function extractAndInsertChecklists(tenant, mediaUrl, _data) {
    ...
    const tasks = await llmExtract(mediaUrl, `הקובץ הוא רשימות תפעוליות של מסעדה (צ׳קליסטים / פתיחה / סגירה / הכנות). חלץ את *כל* המשימות. ` +
        `לכל משימה: checklist (שם הסקשן/הצ׳קליסט שאליו היא שייכת — למשל "פתיחה - עמדה חמה", "סגירה", "הכנות"), text (המשימה עצמה, משפט קצר). ` +
        `שמור על החלוקה לסקשנים בדיוק כפי שמופיעה בקובץ. אם הקובץ באנגלית — שמור אנגלית. אל תמציא ואל תשמיט.`, { checklist: { type: 'string' }, text: { type: 'string' } }, 'tasks', tenant.slug);
    return insertChecklists(tenant, groupTasksIntoChecklists(tasks));
}
// Group a flat [{checklist, text}] list into [{title, category, items[]}].
function groupTasksIntoChecklists(tasks) {
    const groups = new Map();
...
async function insertChecklists(tenant, lists) {
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const cl of lists) {
        ...
        if (!title || !items.length)
            continue;
        await sql(`INSERT INTO "${schema}"."Checklist" ("id", "title", "category", "frequency", "items", "status", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, 'daily', $4::jsonb, 'active', NOW(), NOW())`, await uuid(), title, String(cl?.category || 'כללי'), 
        ...
        }))).then(() => n++).catch((e) => console.warn('[onboarding] checklist insert:', e?.message));
    }
    return n;
}
async function insertSuppliersFromText(tenant, text) {
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const line of text.split(/\n+/).map((l) => l.trim()).filter(Boolean)) {
        ...
        await sql(`INSERT INTO "${schema}"."Supplier" ("id", "company_name", "supplier_id", "contact_person", "email", "phone", "category", "status", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'active', NOW(), NOW())`, await uuid(), name, `SUP-${(n + 1).toString().padStart(3, '0')}-${Date.now() % 10000}`, name, `supplier-${n + 1}@pending.topalena.com`, phone, category).then(() => n++).catch((e) => console.warn('[onboarding] supplier insert:', e?.message));
    }
    return n;
}
async function insertRolesFromText(tenant, text) {
    const names = text.split(/[,\n،]+/).map((r) => r.trim()).filter((r) => r.length >= 2 && r.length <= 40);
    return insertRoles(tenant, names);
}
async function extractAndInsertRolesFromFile(tenant, mediaUrl) {
    const roles = await llmExtract(mediaUrl, `הקובץ הוא סידור עבודה של מסעדה. חלץ את רשימת התפקידים הייחודיים שמופיעים בו (מלצר, טבח, ברמן, אחמ"ש...). לכל תפקיד: name בלבד.`, { name: { type: 'string' } }, 'roles', tenant.slug);
    return insertRoles(tenant, roles.map((r) => String(r?.name || '').trim()).filter(Boolean));
}
...
async function insertRoles(tenant, names) {
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const name of names) {
        if (!name)
            continue;
        const exists = await query(`SELECT 1 FROM "${schema}"."WorkPosition" WHERE position_name = $1 LIMIT 1`, name).catch(() => []);
        if (exists.length)
            continue;
        await sql(`INSERT INTO "${schema}"."WorkPosition" ("id", "position_name", "is_active", "createdAt", "updatedAt")
       VALUES ($1, $2, true, NOW(), NOW())`, await uuid(), name).then(() => n++).catch((e) => console.warn('[onboarding] position insert:', e?.message));
    }
    return n;
}
async function insertEmployeesFromText(tenant, text) {
...
async function extractAndInsertEmployeesFromFile(tenant, mediaUrl) {
    const emps = await llmExtract(mediaUrl, `הקובץ הוא רשימת עובדים של מסעדה (אקסל/טבלה/צילום). חלץ לכל עובד: full_name, role (אם מופיע), phone (אם מופיע), email (אם מופיע). אל תמציא.`, { full_name: { type: 'string' }, role: { type: 'string' }, phone: { type: 'string' }, email: { type: 'string' } }, 'employees', tenant.slug);
    return insertEmployees(tenant, emps);
}
async function insertEmployees(tenant, rows) {
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const r of rows) {
        ...
        if (!fullName || fullName.length < 2)
            continue;
        await sql(`INSERT INTO "${schema}"."Employee" ("id", "full_name", "email", "phone", "role", "status", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, $4, $5, 'active', NOW(), NOW())`, await uuid(), fullName, r?.email && /\S+@\S+/.test(r.email) ? String(r.email).toLowerCase() : `${tenant.slug}-${Date.now() % 100000}-${n}@pending.topalena.com`, r?.phone ? String(r.phone) : null, String(r?.role || 'עובד')).then(() => n++).catch((e) => console.warn('[onboarding] employee insert:', e?.message));
    }
    return n;
}
async function insertInterviewSlots(tenant, text) {
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const line of text.split(/\n+/).map((l) => l.trim()).filter(Boolean)) {
        ...
        if (weekday == null)
            continue;
        await sql(`INSERT INTO "${schema}"."InterviewSlotTemplate" ("id", "weekday", "time", "duration_minutes", "active")
       VALUES ($1, $2, $3, 30, true)`, await uuid(), weekday, m[2]).then(() => n++).catch((e) => console.warn('[onboarding] slot insert:', e?.message));
    }
    return n;
}
async function extractAndInsertCustomers(tenant, mediaUrl) {
    const customers = await llmExtract(mediaUrl, `הקובץ הוא רשימת לקוחות / מועדון לקוחות של מסעדה (טבלה/CSV/צילום). ` +
        `חלץ לכל לקוח: name, phone (חובה — דלג על שורות בלי טלפון), email אם יש, birthday אם יש (פורמט YYYY-MM-DD או DD/MM). אל תמציא.`, { name: { type: 'string' }, phone: { type: 'string' }, email: { type: 'string' }, birthday: { type: 'string' } }, 'customers', tenant.slug);
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const c of customers) {
        ...
        if (phone.length < 9)
            continue;
        const exists = await query(`SELECT 1 FROM "${schema}"."Customer" WHERE phone = $1 LIMIT 1`, phone).catch(() => []);
        if (exists.length)
            continue;
        ...
        await sql(`INSERT INTO "${schema}"."Customer" ("id", "phone", "name", "email", "birthday", "marketing_consent", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, $4, $5, true, NOW(), NOW())`, await uuid(), phone, c?.name ? String(c.name) : null, c?.email && /\S+@\S+/.test(c.email) ? String(c.email).toLowerCase() : null, c?.birthday ? String(c.birthday) : null).then(() => n++).catch((e) => console.warn('[onboarding] customer insert:', e?.message));
    }
    return n;
}
async function extractAndInsertKnowledge(tenant, mediaUrl, category) {
    const entries = await llmExtract(mediaUrl, `הקובץ הוא מסמך ידע של מסעדה (נוהל / מתכון / תפריט / הסכם / שאלות נפוצות). ` +
        `חלץ את התוכן כערכי ידע: לכל נושא — title קצר ו-content מלא ומפורט בעברית (אל תקצר מדי — זה ישמש AI לענות על שאלות). אם המסמך הוא נושא אחד, החזר ערך אחד.`, { title: { type: 'string' }, content: { type: 'string' } }, 'entries', tenant.slug);
    return insertKnowledgeEntries(tenant, entries, category);
}
async function insertKnowledgeEntries(tenant, entries, category) {
    const schema = `tenant_${tenant.slug}`;
    let n = 0;
    for (const e of entries) {
        ...
        if (!title || !content)
            continue;
        await sql(`INSERT INTO "${schema}"."KnowledgeBase" ("id", "category", "title", "content", "priority", "last_updated", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, $4, 'medium', NOW(), NOW(), NOW())`, await uuid(), category, title, content).then(() => n++).catch((err) => console.warn('[onboarding] knowledge insert:', err?.message));
    }
    return n;
}
async function saveInvoiceEmails(tenant, emails) {
    const schema = `tenant_${tenant.slug}`;
    await sql(`ALTER TABLE "${schema}"."RestaurantProfile" ADD COLUMN IF NOT EXISTS "invoice_inbox_emails" TEXT`).catch(() => { });
    const existing = await query(`SELECT id FROM "${schema}"."RestaurantProfile" LIMIT 1`);
    if (existing.length) {
        await sql(`UPDATE "${schema}"."RestaurantProfile" SET invoice_inbox_emails = $1, "updatedAt" = NOW() WHERE id = $2`, emails.join(','), existing[0].id);
    }
    else {
        await sql(`INSERT INTO "${schema}"."RestaurantProfile" ("id", "restaurant_name", "invoice_inbox_emails", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, NOW(), NOW())`, await uuid(), tenant.restaurant_name || tenant.slug, emails.join(','));
    }
}
// --- AI suggestions (when the owner has no file of their own) --------------
async function aiSuggestChecklists(tenant, data, extraContext = '') {
    const { invokeLLM } = await import('./llm.js');
    // Pull the actual menu for precision — what they make reveals equipment/stations.
    let menuHint = '';
    try {
        const schema = `tenant_${tenant.slug}`;
        const cats = await query(`SELECT DISTINCT category FROM "${schema}"."MenuItem" WHERE category IS NOT NULL LIMIT 12`).catch(() => []);
        const names = await query(`SELECT name FROM "${schema}"."MenuItem" ORDER BY "createdAt" LIMIT 12`).catch(() => []);
        const c = cats.map((r) => r.category).filter(Boolean);
        const nm = names.map((r) => r.name).filter(Boolean);
        if (c.length || nm.length)
    ...
    for (const t of TYPES) {
        try {
            const r = await invokeLLM({
                prompt: `בנה צ'קליסט "${t.desc}" ${base}\n` +
            ...
            const items = (Array.isArray(r?.tasks) ? r.tasks : []).map((x) => String(x?.text || '').trim()).filter(Boolean);
            if (items.length)
                lists.push({ title: t.title, category: t.category, items });
        }
        catch (e) {
            console.warn('[onboarding] checklist gen', t.category, e?.message);
        }
    }
    return insertChecklists(tenant, lists);
}
async function aiSuggestRoles(tenant, data) {
    const { invokeLLM } = await import('./llm.js');
    const result = await invokeLLM({
        prompt: `הצע רשימת תפקידים טיפוסית למסעדה מסוג "${data.cuisine || 'כללי'}"` +
    ...
    const roles = (Array.isArray(result?.roles) ? result.roles : []).map((r) => String(r).trim()).filter(Boolean);
...
function splitLessonPoint(pt, order) {
...
async function aiSuggestTraining(tenant, data) {
    const { invokeLLM } = await import('./llm.js');
    const schema = `tenant_${tenant.slug}`;
    ...
    let roles = [];
    try {
        const rows = await query(`SELECT position_name FROM "${schema}"."WorkPosition" WHERE is_active = true ORDER BY "createdAt" LIMIT 6`).catch(() => []);
        roles = rows.map((r) => r.position_name).filter(Boolean);
    }
    catch { /* positions not available */ }
    if (!roles.length)
        roles = ['דלפק ושירות', 'מטבח והכנות', 'בר / ברמנים'];
    const base = `למסעדה מסוג "${data.cuisine || 'כללי'}"${data.description ? ` (${data.description})` : ''}`;
    ...
    for (const role of roles.slice(0, 5)) {
        ...
        try {
            const r = await invokeLLM({
                prompt: `בנה פרק הכשרה לתפקיד "${role}" ${base}.\n` +
            ...
            points = (Array.isArray(r?.points) ? r.points : []).map((x) => String(x?.text || '').trim()).filter(Boolean);
        }
        catch (e) {
            console.warn('[onboarding] training gen', role, e?.message);
        }
        if (!points.length)
            continue;
        const courseCode = `onb-${(await uuid()).slice(0, 8)}`;
        let ok = false;
        await sql(`INSERT INTO "${schema}"."TrainingCourse" ("id","course_code","title","description","category","assigned_role","status","createdAt","updatedAt")
       VALUES ($1,$2,$3,$4,'הכשרה',$5,'published',NOW(),NOW())`, await uuid(), courseCode, `הכשרת ${role}`, `תוכנית הכשרה מותאמת לתפקיד ${role}`, role).then(() => { ok = true; }).catch((err) => console.warn('[onboarding] course insert', role, err?.message));
        if (!ok)
            continue;
        coursesCreated++;
        for (let i = 0; i < points.length; i++) {
            const { title, content } = splitLessonPoint(points[i], i + 1);
            await sql(`INSERT INTO "${schema}"."TrainingLesson" ("id","lesson_id","course_code","title","content_type","content","order","createdAt","updatedAt")
         VALUES ($1,$2,$3,$4,'text',$5,$6,NOW(),NOW())`, await uuid(), await uuid(), courseCode, title, content, i + 1).catch((err) => console.warn('[onboarding] lesson insert', err?.message));
        }
    }
...
async function extractSeatingTables(tenant, mediaUrl) {
    // property `tables`, NOT `items` — collision-safe.
    const rows = await llmExtract(mediaUrl, `הקובץ הוא מפת הושבה / סקיצה של מסעדה. חלץ את השולחנות: label (מספר/שם), capacity (מספר סועדים), x ו-y (מיקום יחסי 0-100), shape (table/bar/booth/outdoor). אל תמציא.`, { label: { type: 'string' }, capacity: { type: 'number' }, x: { type: 'number' }, y: { type: 'number' }, shape: { type: 'string' } }, 'tables', tenant.slug);
    return rows.map((t, i) => ({
...
async function insertSeatingTables(tenant, tables) {
    if (!Array.isArray(tables) || !tables.length)
        return 0;
    const schema = `tenant_${tenant.slug}`;
    try {
        const existing = await query(`SELECT id FROM "${schema}"."SeatingLayout" LIMIT 1`);
        if (existing.length) {
            await sql(`UPDATE "${schema}"."SeatingLayout" SET tables = $1::jsonb, "updatedAt" = NOW() WHERE id = $2`, JSON.stringify(tables), existing[0].id);
        }
        else {
            await sql(`INSERT INTO "${schema}"."SeatingLayout" ("id", "layout_name", "tables", "createdAt", "updatedAt") VALUES ($1, 'מפה ראשית', $2::jsonb, NOW(), NOW())`, await uuid(), JSON.stringify(tables));
        }
    }
    catch (e) {
...
function seatingSummary(tables) {
    const byArea = {};
    let cap = 0;
...
async function extractAndInsertSeating(tenant, mediaUrl) {
...
export async function isOnboardingActive(fromPhone) {
    const s = await findActiveOnboarding(fromPhone).catch(() => null);
    return !!s;
}
export async function ensureOnboardingRow(tenantId) {
    const { randomUUID } = await import('node:crypto');
    await sql(`INSERT INTO "OnboardingState" ("id", "tenant_id") VALUES ($1, $2)
     ON CONFLICT (tenant_id) DO NOTHING`, randomUUID(), tenantId);
}
async function getTenant(tenantId) {
    const rows = await query(`SELECT id, slug, restaurant_name, owner_name, owner_phone, owner_email, status
     FROM "Tenant" WHERE id = $1`, tenantId);
    return rows[0] || null;
}
async function findActiveOnboarding(fromPhone) {
    const digits = String(fromPhone || '').replace(/\D/g, '').replace(/^0/, '972');
    const variants = [fromPhone, digits, `0${digits.replace(/^972/, '')}`, `+${digits}`];
    const rows = await query(`SELECT os.tenant_id, os.current_step, os.completed_steps, os.collected_data
...
function buildDoneMessage(tenant, data) {
...
//# sourceMappingURL=whatsappOnboarding.js.map