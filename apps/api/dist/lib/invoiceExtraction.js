...
export async function fuzzyFindSupplier(name, taxId) {
    // Exact tax-id match wins immediately.
    if (taxId) {
        const exact = await prisma.supplier.findFirst({ where: { supplier_id: String(taxId).trim() } }).catch(() => null);
        if (exact)
            return { match: exact, confidence: 1 };
    }
    const all = await prisma.supplier.findMany({ take: 500 }).catch(() => []);
    let best = null;
    for (const s of all) {
...
export async function extractInvoiceFromFile(fileUrl) {
...
export async function extractInvoicesFromFile(fileUrl) {
    ...
    return list.filter(x => x && x.supplier_name && x.total_amount != null);
}
...
function inventorySimilarity(a, b) {
    const A = normalizeForMatch(a);
    const B = normalizeForMatch(b);
    if (!A || !B)
        return 0;
    ...
    if (substr && Math.abs(At.length - Bt.length) <= 1)
        return 0.92;
...
export function matchInventoryItem(productName, aliases, inventoryItems) {
    const norm = normalizeForMatch(productName);
    const alias = aliases.find(a => a.alias_name === norm);
    if (alias)
        return { action: 'add_existing', inventory_item_id: alias.inventory_item_id, via: 'alias' };
    let best = null;
    ...
    if (best && best.sim >= 0.75)
        return { action: 'add_existing', inventory_item_id: best.id, via: 'fuzzy' };
    return { action: 'create_new' };
}
// DB wrapper used by the scan job.
export async function suggestInventoryAction(productName) {
...
//# sourceMappingURL=invoiceExtraction.js.map