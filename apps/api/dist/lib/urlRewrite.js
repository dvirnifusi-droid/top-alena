...
export function rewriteFileUrl(v) {
    if (typeof v !== 'string' || v.length < 8)
        return v;
    const m1 = v.match(INTERNAL_FILE_RE);
    if (m1 && INTERNAL_HOST_RE.test(m1[1]))
        return `/api/files/${m1[2]}`;
    const m2 = v.match(STORAGE_PATH_RE);
    if (m2)
        return `/api/files/${m2[1]}`;
    return v;
}
...
export function rewriteFileUrlsDeep(v) {
    if (typeof v === 'string')
        return rewriteFileUrl(v);
    if (Array.isArray(v))
        return v.map(rewriteFileUrlsDeep);
