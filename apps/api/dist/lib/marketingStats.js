...
const dbx = () => prisma;
export async function marketingStats(days = 30) {
    const window = Math.max(1, Math.min(365, Math.round(days)));
    const rows = await dbx().$queryRawUnsafe(`SELECT s.id, s.campaign_label, s.campaign_key, s.channel, s.sent_at,
...
//# sourceMappingURL=marketingStats.js.map