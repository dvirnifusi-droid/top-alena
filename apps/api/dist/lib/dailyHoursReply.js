...
export async function tryHandleDailyHoursReply(fromPhone, body) {
    try {
...
async function handleDailyHoursReplyInner(fromPhone, body) {
    ...
    if (!sentAt || Date.now() - sentAt > RECENT_MS)
        return null; // stale — not a reply to a live report
    // ── Approval ──
    if (APPROVE_RE.test(text)) {
        ...
        const flagged = snap.rows.filter(r => r.flags?.length).length;
    ...
    if (!corrections.length)
        return null; // not an approval, not a correction → let normal flow handle it
    ...
    for (const c of corrections) {
        ...
        if (matches.length === 0) {
            problems.push(`"${c.name}" — לא בדוח`);
            continue;
        }
        if (matches.length > 1) {
            problems.push(`"${c.name}" — יותר מאחד (${matches.map(m => m.name).join(', ')})`);
            continue;
        }
        const row = matches[0];
