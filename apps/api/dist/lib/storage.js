...
const publicUrl = rawPublic && !/localhost|127\.0\.0\.1|minio:|:9000/.test(rawPublic) ? rawPublic : '/api/files';
...
export async function createSignedUrl(fileUrl, expiresIn = 3600) {
    await ensureBucket();
    // Extract the key from the public URL (works for default MinIO/S3 URLs).
    const prefix = publicUrl + '/';
    if (!fileUrl?.startsWith(prefix))
        return { url: fileUrl, signed: false };
...
export async function uploadStreamToS3(filename, mimetype, stream) {
    await ensureBucket();
