...
function ymd(d = new Date()) { return d.toLocaleDateString('en-CA', { timeZone: TZ }); }
function israelDayName(ymdStr) {
    const NAMES = ['ראשון', 'שני', 'שלישי', 'רביעי', 'חמישי', 'שישי', 'שבת'];
    try {
        return NAMES[new Date(`${ymdStr}T12:00:00.000Z`).getDay()];
    }
    catch {
        return '';
    }
...
async function loadHistory(phone) {
    ...
    const turns = [];
    for (const r of rows) {
        if (!r.body || String(r.body).startsWith('🎙️'))
            continue; // skip transcription echoes
        turns.push({
    ...
    for (const t of chrono) {
        if (String(t.text).startsWith('[pending]'))
            continue; // internal stash rows
        if (seq.length && seq[seq.length - 1].role === t.role) {
            seq[seq.length - 1] = t;
            continue;
        }
        seq.push(t);
    }
    while (seq.length && seq[0].role === 'model')
        seq.shift();
...
const RES_toMin = (t) => { const [h, m] = String(t).split(':').map(Number); return h * 60 + m; };
const RES_seatingDuration = (size) => (size >= 11 ? 150 : size >= 6 ? 135 : 120);
const RES_isValidTime = (t) => /^([01]?\d|2[0-3]):[0-5]\d$/.test(String(t || '').trim());
async function tool_check_availability(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    const size = parseInt(String(args.party_size), 10);
    if (!Number.isFinite(size) || size < 1)
        return { error: 'צריך מספר סועדים תקין.' };
    const dateStr = resolveDate(String(args.date || ''));
    if (!dateStr)
        return { error: `לא הצלחתי לפענח את התאריך "${args.date}".` };
    const time = String(args.time || '').trim();
    if (!time)
        return { need_time: true, message: 'לאיזו שעה לבדוק זמינות? (למשל 20:00)' };
    if (!RES_isValidTime(time))
        return { error: `שעה לא תקינה "${args.time}" — פורמט HH:MM.` };
    const settings = await prisma.reservationSettings.findFirst().catch(() => null);
    ...
    const active = (r) => r.status !== 'cancelled' && r.status !== 'no_show';
    const startMin = RES_toMin(time);
    const slotCount = reservations
        .filter((r) => active(r) && r.time && RES_toMin(r.time) >= startMin && RES_toMin(r.time) < startMin + 15)
...
async function tool_propose_create_reservation(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
...
async function tool_propose_cancel_reservation(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!nameQ && !dateStr)
        return { error: 'ציין שם לקוח ו/או תאריך של ההזמנה לביטול.' };
    ...
    let candidates = await prisma.reservation.findMany({ where, take: 100 });
    if (nameQ)
        candidates = candidates.filter((r) => String(r.customer_name || '').toLowerCase().includes(nameQ));
    candidates.sort((a, b) => {
    ...
    if (!candidates.length)
        return { not_found: true, message: 'לא נמצאה הזמנה פעילה שתואמת לחיפוש.' };
    if (candidates.length > 1) {
        return {
            ambiguous: true,
            message: 'נמצאו כמה הזמנות — על איזו מדובר?',
            candidates: candidates.slice(0, 8).map((r) => ({
...
// ─── Revenue / ordering / tips (manager-level) ─────────────────────────────
async function tool_get_today_revenue(_args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    const [bc, gm] = await Promise.all([
        prisma.beecommSnapshot.findFirst({ orderBy: { captured_at: 'desc' } }).catch(() => null),
        prisma.gomileySnapshot.findFirst({ orderBy: { captured_at: 'desc' } }).catch(() => null),
    ]);
...
async function tool_list_order_needs(_args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    const prepRows = await prisma.$queryRawUnsafe(`SELECT pi."name" AS name, pi."category" AS category, pi."unit" AS unit,
     ...
     ORDER BY pl."sort" ASC, pi."sort" ASC`).catch(() => []);
    const suppliers = await prisma.$queryRawUnsafe(`SELECT "id","name","category","items","last_ordered_at"
     FROM "SupplierOrderList" ORDER BY "sort" ASC, "name" ASC`).catch(() => []);
    const TWO_DAYS = 2 * 24 * 60 * 60 * 1000;
    const suppliers_to_order = suppliers.map((s) => {
        const items = Array.isArray(s.items) ? s.items : [];
        const toOrder = items.filter((it) => {
            const par = Number(it.par);
            const cur = Number(it.current);
    ...
    }).filter((s) => !s.recently_ordered && s.to_order_count > 0);
    return {
        prep_order_items: prepRows.map((r) => ({
...
async function tool_propose_mark_supplier_ordered(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!name)
        return { error: 'איזה ספק סימנת שהזמנת ממנו?' };
    const rows = await prisma.$queryRawUnsafe(`SELECT "id","name","last_ordered_at" FROM "SupplierOrderList"
     WHERE "name" ILIKE $1 OR "name" ILIKE $2 ORDER BY "sort" ASC LIMIT 6`, `%${name}%`, `${name}%`).catch(() => []);
    if (!rows.length)
        return { found: false, message: `לא מצאתי ספק בשם "${name}". נסה שם אחר או פתח את רשימת הספקים.` };
    if (rows.length > 1) {
        return { ambiguous: true, message: 'יש כמה ספקים תואמים — לאיזה התכוונת?', options: rows.map((r) => r.name) };
    }
    const sup = rows[0];
...
async function tool_propose_lock_tips(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!date)
        return { error: 'לא הבנתי את התאריך. נסה "אתמול" או תאריך בפורמט YYYY-MM-DD.' };
    ...
    if (!reports.length)
        return { found: false, message: `לא נמצא דו"ח טיפים ל-${date}${shiftHe}.` };
    const already = reports.filter((r) => r.status === 'locked').length;
    const toLock = reports.length - already;
    if (toLock === 0)
        return { message: `כל דו"חות הטיפים של ${date}${shiftHe} כבר נעולים 🔒` };
    const total = reports.reduce((s, r) => s + Number(r.total_tips_collected || 0), 0);
    await stashPendingAction(phone, {
...
// ─── Incidents / expenses / dish pricing (manager-level) ───────────────────
async function tool_propose_open_incident(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
...
async function tool_list_incidents(_args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
...
async function tool_propose_resolve_incident(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!q)
        return { error: 'איזו תקלה לסגור? תן תיאור קצר.' };
    ...
    const matches = open.filter((i) => String(i.title || '').toLowerCase().includes(ql) ||
        String(i.description || '').toLowerCase().includes(ql) ||
        (idq.length >= 4 && i.id.slice(-6).toLowerCase() === idq));
    if (!matches.length)
        return { error: `לא מצאתי תקלה פתוחה שמתאימה ל-"${q}".` };
    if (matches.length > 1)
        return {
            ambiguous: true,
            candidates: matches.slice(0, 6).map((i) => `${i.title || (i.description || '').slice(0, 40)} (${i.id.slice(-6)})`),
        };
    const inc = matches[0];
...
async function tool_propose_mark_expense_paid(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    const q = String(args.match || '').trim();
    if (!q)
        return { error: 'איזו הוצאה לסמן כשולמה? תן שם ספק או תיאור.' };
    const rows = await prisma.$queryRawUnsafe(`SELECT id, date::date AS date, source, description, category, amount
     FROM "CashFlowEntry"
     WHERE type = 'expense' AND status = 'planned'
       AND (source ILIKE $1 OR description ILIKE $1 OR category ILIKE $1)
     ORDER BY date ASC LIMIT 6`, `%${q}%`);
    if (!rows.length)
        return { error: `לא מצאתי הוצאה פתוחה שמתאימה ל-"${q}".` };
    if (rows.length > 1)
        return {
            ambiguous: true,
            candidates: rows.map((r) => `${r.source || r.description || r.category} — ₪${Math.round(Number(r.amount))} (${r.date instanceof Date ? r.date.toISOString().slice(0, 10) : r.date})`),
        };
    const e = rows[0];
...
async function tool_propose_set_dish_price(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!name)
        return { error: 'איזו מנה לעדכן?' };
    if (!Number.isFinite(price) || price <= 0)
        return { error: 'צריך מחיר חיובי בשקלים.' };
    const rows = await prisma.$queryRawUnsafe(`SELECT id, name, kind, sale_price FROM "Recipe"
     WHERE (name ILIKE $1 OR name ILIKE $2) AND kind = 'DISH' LIMIT 5`, `%${name}%`, `${name}%`);
    if (!rows.length)
        return { error: `לא מצאתי מנה בשם "${name}".` };
    if (rows.length > 1)
        return {
            ambiguous: true,
            candidates: rows.map((r) => `${r.name}${r.sale_price ? ` (נוכחי ₪${r.sale_price})` : ''}`),
        };
    const rec = rows[0];
    await stashPendingAction(phone, {
...
async function tool_propose_team_broadcast(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.is_owner)
        return { error: 'רק הבעלים יכול לשלוח הודעה לכל הצוות.' };
    const message = String(args.message || '').trim();
    if (message.length < 3)
        return { error: 'צריך תוכן הודעה.' };
    ...
    if (!recipients.length)
        return { error: `לא נמצאו עובדים עם טלפון לקהל המבוקש (${audience === 'tonight' ? 'משובצי היום' : department || 'כולם'}).` };
    await stashPendingAction(phone, { type: 'team_broadcast', message, audience, department: department || '', target_phone: phone });
    const sample = recipients.slice(0, 6).map((r) => r.full_name).join(', ');
...
async function tool_propose_invite_employee(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    if (!scope.is_owner)
        return { error: 'רק הבעלים יכול לבצע פעולה זו.' };
    const fullName = String(args.full_name || '').trim();
    const empPhone = String(args.phone || '').replace(/[^\d+]/g, '');
    if (fullName.length < 2)
        return { error: 'צריך שם מלא של העובד.' };
    if (empPhone.replace(/\D/g, '').length < 9)
        return { error: 'מספר טלפון לא תקין. שלח מספר עם קידומת.' };
    const existing = await prisma.employee.findFirst({ where: { phone: empPhone } }).catch(() => null);
    if (existing)
        return { error: `כבר קיים עובד עם הטלפון ${empPhone} (${existing.full_name}).` };
...
async function tool_propose_remove_from_shift(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!m.exact.length) {
        if (m.suggestions.length)
            return {
                ...
                suggestions: m.suggestions.map((e) => e.full_name),
            };
        return { error: `לא נמצא עובד פעיל בשם "${args.employee_name}".` };
    }
    if (m.exact.length > 1)
        return { ambiguous: true, candidates: m.exact.map((e) => e.full_name) };
    const emp = m.exact[0];
    ...
    if (!shift || !(shift.assigned_staff || []).some((a) => a.employee_id === emp.id)) {
...
async function tool_propose_publish_schedule(_args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!empIds.size)
        return { error: 'אין משמרות משובצות לשבוע הבא — בנה את הסידור קודם.' };
...
async function tool_propose_approve_event(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.can_write)
        return { error: 'הפעולה הזו דורשת הרשאת מנהל.' };
    ...
    if (!q)
        return { error: 'למי לאשר את האירוע? שלח שם לקוח.' };
    const bookings = await prisma.eventBooking.findMany({ take: 500 });
    const pending = bookings.filter((b) => b.approval_status !== 'approved' && b.approval_status !== 'rejected');
    if (!pending.length)
        return { error: 'אין הזמנות אירוע ממתינות לאישור.' };
    const asEmp = pending.map((b) => ({ ...b, full_name: b.customer_name || '' }));
    let hit = matchEmployees(q, asEmp).exact;
    if (!hit.length)
        hit = matchEmployees(q, asEmp).suggestions;
    if (!hit.length)
        return { error: `לא מצאתי הזמנת אירוע ממתינה בשם "${q}".` };
    if (hit.length > 1)
        return {
            ambiguous: true,
            message: 'יש כמה הזמנות ממתינות תואמות — איזו?',
            candidates: hit.map((b) => `${b.customer_name} · ${b.event_date}${b.guest_count ? ` · ${b.guest_count} סועדים` : ''}`),
        };
    const b = hit[0];
...
async function tool_list_today_schedule(args, _phone) {
    ...
    const out = { date: targetYMD, day_name: israelDayName(targetYMD), lunch: [], dinner: [] };
    for (const t of ['lunch', 'dinner']) {
        const matching = shifts.filter((s) => s.shift_type === t);
        const staff = matching.flatMap((s) => s.assigned_staff || []);
        out[t] = staff.map((a) => ({ name: a.employee_name, position: a.position, time: `${a.start_time}-${a.end_time}` }));
    }
    return out;
}
async function tool_list_today_events(args, phone) {
    const targetYMD = args?.date || ymd();
    ...
    const events = rows
        .map((r) => ({ at: new Date(r.raw?.event_at || 0), title: r.raw?.title || r.body }))
        .filter((e) => e.at.toLocaleDateString('en-CA', { timeZone: TZ }) === targetYMD)
        .sort((a, b) => a.at.getTime() - b.at.getTime());
    return { date: targetYMD, events: events.map((e) => ({ time: e.at.toLocaleString('he-IL', { timeZone: TZ, hour: '2-digit', minute: '2-digit' }), title: e.title })) };
}
async function tool_list_open_tasks(_args, phone) {
...
async function tool_list_open_leads(_args, _phone) {
...
async function tool_get_recent_tips(args, _phone) {
    const days = Math.max(1, Math.min(30, Number(args?.days) || 7));
    const since = new Date(Date.now() - days * 86_400_000);
    const reports = await prisma.tipReport.findMany({ orderBy: { date: 'desc' }, take: 50 });
    const inRange = reports.filter((r) => new Date(r.date).getTime() >= since.getTime());
    const total = inRange.reduce((s, r) => s + Number(r.total_tips_collected || 0), 0);
...
async function tool_get_unpaid_invoices(_args, _phone) {
    const inv = await prisma.invoice.findMany({ where: { payment_status: 'unpaid' }, orderBy: { invoice_date: 'desc' }, take: 200 });
    ...
    const sups = {};
    if (recent.length) {
        const supIds = [...new Set(recent.map((i) => i.supplier_id).filter(Boolean))];
        const ss = await prisma.supplier.findMany({ where: { id: { in: supIds } } }).catch(() => []);
        for (const s of ss)
...
async function tool_search_employee(args, _phone) {
    ...
    return {
        ...
        matches: picked.slice(0, 8).map((e) => ({
            id: e.id, name: e.full_name, phone: e.phone,
            positions: (e.positions || []).map((p) => p?.position_name || p).filter(Boolean),
        })),
    };
}
async function tool_search_lead(args, _phone) {
    const q = String(args?.query || '').toLowerCase().trim();
    if (!q)
        return { matches: [] };
    const leads = await prisma.eventLead.findMany({ orderBy: { id: 'desc' }, take: 200 });
    const matches = leads.filter((l) => String(l.contact_name || '').toLowerCase().includes(q) ||
        String(l.contact_phone || '').includes(q.replace(/\D/g, '')));
    return { matches: matches.slice(0, 8).map((l) => ({
...
async function tool_search_invoice(args, _phone) {
    const q = String(args?.query || '').toLowerCase().trim();
    if (!q)
        return { matches: [] };
    const inv = await prisma.invoice.findMany({ orderBy: { invoice_date: 'desc' }, take: 500 });
    // Match by invoice_number prefix OR by supplier company_name fragment
    const sups = await prisma.supplier.findMany({ take: 500 });
    const matchingSupIds = new Set(sups.filter((s) => String(s.company_name || '').toLowerCase().includes(q)).map((s) => s.id));
    const matches = inv.filter((i) => String(i.invoice_number || '').toLowerCase().includes(q) ||
...
async function tool_propose_event_add(args, phone) {
    const when = new Date();
    ...
    if (!exec.when)
        return { error: `couldn't parse time '${args.when}'` };
...
async function tool_propose_task_add(args, phone) {
...
async function tool_propose_task_done(args, phone) {
...
async function tool_propose_lead_set_stage(args, phone) {
    const leads = await prisma.eventLead.findMany({ where: { status: { in: ['pending', 'contacted', 'quoted'] } }, take: 100 });
    const q = String(args.lead_query).toLowerCase();
    const matches = leads.filter((l) => String(l.contact_name || '').toLowerCase().includes(q) ||
        String(l.contact_phone || '').includes(q.replace(/\D/g, '')));
    if (!matches.length)
        return { error: `no matching lead for "${args.lead_query}"` };
    if (matches.length > 1)
        return { ambiguous: true, candidates: matches.map((l) => ({ name: l.contact_name, phone: l.contact_phone })) };
...
async function tool_propose_invoice_mark_paid(args, phone) {
    const inv = await prisma.invoice.findFirst({ where: { invoice_number: args.invoice_number } });
    if (!inv)
        return { error: `no invoice with number ${args.invoice_number}` };
    if (inv.payment_status === 'paid')
        return { error: `invoice ${args.invoice_number} already paid` };
...
async function tool_propose_shift_assign(args, phone) {
    ...
    const dateStr = resolveDate(args.date);
    if (!dateStr)
        return { error: `couldn't parse date "${args.date}"` };
...
async function tool_propose_remind_me(args, phone) {
...
async function tool_propose_event_add_batch(args, phone) {
    const items = Array.isArray(args.events) ? args.events : [];
    if (!items.length)
    ...
    for (const it of items) {
        if (!it?.title || !it?.when) {
            skipped.push(`${it?.title || '?'} (חסר זמן)`);
            continue;
        }
        const iso = tryParseTimestamp(it.when);
        if (!iso) {
            skipped.push(`${it.title} (${it.when} לא ניתן לפענוח)`);
            continue;
        }
        parsed.push({
    ...
    if (!parsed.length)
        return { error: 'could not parse any event times', skipped };
    ...
    if (skipped.length)
        summary.push(``, `⚠️ דילגתי: ${skipped.join(' · ')}`);
    return { proposal: summary.join('\n'), awaiting_confirmation: true };
}
async function tool_propose_employee_shifts_batch(args, phone) {
    const search = String(args.employee_search || '').trim();
    if (!search)
        return { error: 'employee_search required' };
    ...
    if (m.exact.length > 1)
        return {
            ambiguous: true,
            candidates: m.exact.slice(0, 6).map((e) => e.full_name),
        };
    const emp = m.exact[0];
    ...
    for (const e of rawEntries) {
        ...
        if (!ns || !ne) {
            errors.push(`bad time ${start}-${end}`);
            continue;
        }
        const startH = parseInt(ns.split(':')[0]);
        const shift_type = startH < 16 ? 'lunch' : 'dinner';
        parsed.push({
            date: ymd,
            start: ns,
            end: ne,
    ...
    if (!parsed.length)
        return { error: 'no parseable entries', errors };
    ...
    for (const e of parsed) {
        ...
        const existing = dayShifts.find((s) => (s.assigned_staff || []).some((a) => a.employee_id === emp.id));
        if (existing) {
            const cur = (existing.assigned_staff || []).find((a) => a.employee_id === emp.id);
            conflicts.push({
                ...e,
    ...
    if (fresh.length) {
        ...
        for (const f of fresh)
            lines.push(`  • ${f.date.slice(8) + '.' + f.date.slice(5, 7)} · ${f.start}-${f.end} (${f.shift_type === 'lunch' ? 'צהריים' : 'ערב'})`);
    }
    if (conflicts.length) {
        const targetPos = explicitPosition || '';
        const identicalConflicts = conflicts.filter((c) => c.existing_start === c.start && c.existing_end === c.end && (!targetPos || c.existing_position === targetPos));
        const realChanges = conflicts.filter((c) => !identicalConflicts.includes(c));
        ...
        if (identicalConflicts.length) {
            lines.push('', `✓ *${identicalConflicts.length} כבר זהות (אין מה לעדכן):* ${identicalConflicts.map((c) => c.date.slice(8) + '.' + c.date.slice(5, 7)).join(', ')}`);
        }
    }
    if (errors.length)
        lines.push('', `❌ שגיאות parsing: ${errors.join(' · ')}`);
...
async function tool_list_pending_proposals(_args, phone) {
    ...
    return { count: rows.length, proposals: rows.map((r) => ({
            id: r.id.slice(-6), status: r.status, summary: String(r.body || '').slice(0, 200),
            exec: r.raw?.pending_action || r.raw?.pending_invoice || null,
        })) };
}
async function tool_modify_pending_proposal(args, phone) {
    const since = new Date(Date.now() - 15 * 60 * 1000);
    ...
    if (!pending)
        return { error: 'no_pending_proposal' };
    ...
    if (args.when !== undefined) {
        ...
        if (!iso)
            return { error: `couldn't parse new time '${args.when}'` };
    ...
    if (args.title !== undefined) {
        if (exec.type === 'event_add' || exec.type === 'task_add')
            exec.title = args.title;
        if (exec.type === 'remind_me')
            exec.remind_text = args.title;
    }
    if (args.lead_min !== undefined && exec.type === 'event_add')
        exec.lead_min = args.lead_min;
    if (args.stage !== undefined && exec.type === 'lead_set_stage')
        exec.stage = args.stage;
    if (args.invoice_number !== undefined && (exec.type === 'invoice_mark_paid' || exec.type === 'invoice_mark_unpaid')) {
        // resolve invoice by new number
        const inv = await prisma.invoice.findFirst({ where: { invoice_number: args.invoice_number } });
        if (!inv)
            return { error: `no invoice with number ${args.invoice_number}` };
        exec.invoice_id = inv.id;
        exec.invoice_number = args.invoice_number;
    }
    await prisma.whatsAppMessage.update({
        where: { id: pending.id },
        data: { raw: { pending_action: exec }, created_at: new Date() }, // reset clock — fresh 10 min window
    });
...
async function tool_cancel_pending_proposal(_args, phone) {
...
async function tool_update_scheduled_event(args, phone) {
    const match = String(args.title_match || '').toLowerCase();
    if (!match)
        return { error: 'title_match required' };
    const rows = await prisma.whatsAppMessage.findMany({
        where: { status: 'scheduled_event', contact_phone: phone, is_read: false },
        orderBy: { id: 'desc' }, take: 50,
    });
    const matches = rows.filter((r) => String(r.raw?.title || '').toLowerCase().includes(match));
    if (!matches.length)
        return { error: `no scheduled event matching "${args.title_match}"` };
    if (matches.length > 1)
        return {
            ambiguous: true,
            candidates: matches.slice(0, 5).map((r) => ({
                title: r.raw?.title,
                when: new Date(r.raw?.event_at).toLocaleString('he-IL', { timeZone: TZ, dateStyle: 'short', timeStyle: 'short' }),
            })),
        };
    const ev = matches[0];
    const raw = { ...ev.raw };
    if (args.new_when) {
        ...
        if (!hasDateHint && timeOnly) {
            ...
            if (!iso)
                return { error: `couldn't parse new time '${args.new_when}'` };
            raw.event_at = iso;
        }
    ...
    if (args.new_title)
        raw.title = args.new_title;
    if (args.new_lead_min != null)
        raw.lead_min = args.new_lead_min;
    await prisma.whatsAppMessage.update({ where: { id: ev.id }, data: { raw } });
...
async function tool_cancel_scheduled_event(args, phone) {
    const match = String(args.title_match || '').toLowerCase();
    if (!match)
        return { error: 'title_match required' };
    const rows = await prisma.whatsAppMessage.findMany({
    ...
    if (matches.length > 1)
        return {
            ambiguous: true,
            candidates: matches.slice(0, 5).map((r) => ({ title: r.raw?.title })),
        };
    const ev = matches[0];
    await prisma.whatsAppMessage.update({
        where: { id: ev.id },
    ...
    return { cancelled: true, title: ev.raw?.title };
}
// ─── Helpers ─────────────────────────────────────────────────────────────
...
function dateAtIsraelLocal(targetYmd, h, m) {
...
function vagueTimeToHHMM(s) {
    if (/בבוקר|morning/i.test(s))
        return { h: 9, m: 0 };
    if (/בצהריים|בצהרים|noon|afternoon/i.test(s))
        return { h: 13, m: 0 };
    if (/אחה"?צ|אחר[\s-]?הצהריים/i.test(s))
        return { h: 16, m: 0 };
    if (/בערב|evening/i.test(s))
        return { h: 19, m: 0 };
    if (/בלילה|night/i.test(s))
        return { h: 22, m: 0 };
    return null;
}
function tryParseTimestamp(raw) {
    ...
    if (relHe || relEn) {
        ...
        const u = m[2];
        let ms = 60_000;
        if (/שעה|שעות|hour/i.test(u))
            ms = 3600_000;
        else if (/יום|ימים|day/i.test(u))
            ms = 86_400_000;
        else if (/שני|sec/i.test(u))
            ms = 1000;
    ...
    if (hhmm) {
        ...
        if (/מחר|tomorrow/.test(s)) {
            ...
            const HE = { 'ראשון': 0, 'שני': 1, 'שלישי': 2, 'רביעי': 3, 'חמישי': 4, 'שישי': 5, 'שבת': 6 };
...
function resolveDate(raw) {
    ...
    if (/^(היום|today)$/.test(s))
        return tzNow.toLocaleDateString('en-CA', { timeZone: TZ });
    if (/^(מחר|tomorrow)$/.test(s)) {
        const t = new Date(tzNow);
        t.setDate(t.getDate() + 1);
...
async function stashPendingAction(phone, exec) {
    await prisma.whatsAppMessage.create({
        data: {
            ...
            raw: { pending_action: exec },
            is_read: false,
        },
    }).catch(() => { });
}
// ─── Recipe / food-cost tools (admin only) ────────────────────────────────
async function tool_get_recipe_cost(args, _phone) {
    const name = String(args.name || '').trim();
    if (!name)
        return { error: 'name required' };
    const rows = await prisma.$queryRawUnsafe(`SELECT id, name, kind, total_cost, sale_price, food_cost_percent, category
     FROM "Recipe" WHERE name ILIKE $1 OR name ILIKE $2 LIMIT 5`, `%${name}%`, `${name}%`);
    if (!rows.length)
        return { found: false, message: 'לא נמצאה מנה בשם הזה' };
    return {
        found: true,
        matches: rows.map((r) => ({
            name: r.name,
...
async function tool_list_high_food_cost(args, _phone) {
    const threshold = Number(args.threshold || 35);
    const rows = await prisma.$queryRawUnsafe(`SELECT name, total_cost, sale_price, food_cost_percent
     FROM "Recipe" WHERE kind = 'DISH' AND food_cost_percent > $1
     ORDER BY food_cost_percent DESC LIMIT 30`, threshold);
    return {
...
async function tool_update_ingredient_price(args, _phone) {
    const name = String(args.name || '').trim();
    const price = Number(args.new_price);
    if (!name || !Number.isFinite(price) || price <= 0)
        return { error: 'name and positive new_price required' };
    const rows = await prisma.$queryRawUnsafe(`SELECT id, name, price_per_unit, unit, supplier_name FROM "Ingredient"
     WHERE name ILIKE $1 OR name ILIKE $2 LIMIT 5`, `%${name}%`, `${name}%`);
    if (!rows.length)
        return { error: 'לא נמצא רכיב בשם הזה' };
    if (rows.length > 1) {
        return {
            ambiguous: true,
    ...
    await prisma.$executeRawUnsafe(`UPDATE "Ingredient" SET price_per_unit = $1, "updatedAt" = NOW() WHERE id = $2`, price, target.id);
    // Ripple to all recipes (cheap — ~80 ingredients × few queries each).
    const { default: load } = await import('../functions/load.js').then(m => ({ default: m.recomputeAllRecipeCosts || (() => Promise.resolve()) })).catch(() => ({ default: () => Promise.resolve() }));
    try {
        await load();
    }
    catch { /* helper not exported — silently skip ripple */ }
    return {
        ok: true,
...
// ─── Cash flow tools (admin / manager) ──────────────────────────────────
async function tool_get_cash_balance(args, _phone) {
    const days = Math.min(60, parseInt(String(args.days || 14)) || 14);
    const horizon = new Date(Date.now() + days * 86400 * 1000);
    const rows = await prisma.$queryRawUnsafe(`SELECT date::date AS date, type, amount FROM "CashFlowEntry"
...
async function tool_list_unpaid_expenses(_args, _phone) {
    const rows = await prisma.$queryRawUnsafe(`SELECT id, date::date AS date, source, category, amount, payment_method
     FROM "CashFlowEntry"
...
async function tool_list_expected_income(_args, _phone) {
    const rows = await prisma.$queryRawUnsafe(`SELECT date::date AS date, source, category, amount
     FROM "CashFlowEntry"
...
async function tool_get_my_recipe_summary(_args, _phone) {
    const dishes = await prisma.$queryRawUnsafe(`SELECT COUNT(*)::int AS n, AVG(food_cost_percent)::float AS avg_fc,
...
async function tool_get_my_schedule(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    ...
    for (const ws of shifts) {
        ...
        const me = staff.find((a) => a?.employee_id === scope.employee_id);
        if (me)
            mine.push({
...
async function tool_get_my_tips(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.employee_id)
        return { error: 'unauthenticated' };
    ...
    for (const r of reports) {
        ...
        const me = sd.find((s) => s?.employee_id === scope.employee_id ||
...
async function tool_get_my_hours(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.employee_id)
        return { error: 'unauthenticated' };
    ...
    const tracking = await prisma.$queryRaw `
...
async function tool_propose_mark_sick(args, phone) {
    const { resolveAccessScope } = await import('./whatsappPermissions.js');
    const scope = await resolveAccessScope(phone);
    if (!scope.employee_id)
        return { error: 'unauthenticated' };
    ...
    const today = new Date(new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Jerusalem' }) + 'T00:00:00.000Z');
    let when = today;
    if (dateRaw === 'tomorrow' || dateRaw === 'מחר')
        when = new Date(today.getTime() + 86400 * 1000);
...
async function tool_build_schedule_now(_args, _phone) {
    const { runWeeklyScheduleBuild } = await import('../functions/load.js');
    const res = await runWeeklyScheduleBuild({ force: true });
    if (res?.skipped)
...
async function tool_add_scheduling_rule(args, _phone) {
    const description = String(args.description || '').trim();
    if (!description)
        return { error: 'description required' };
    const kind = String(args.kind || 'note');
    const { randomUUID } = await import('node:crypto');
    await prisma.$executeRawUnsafe(`INSERT INTO "SchedulingRule"(
       "id","kind","employee_name","day_of_week","shift_type",
       "other_employee_name","max_per_week","description"
     ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, randomUUID(), kind, args.employee_name || null, args.day_of_week != null ? args.day_of_week : null, args.shift_type || null, args.other_employee_name || null, args.max_per_week != null ? args.max_per_week : null, description);
    return { ok: true, message: `✅ נוסף חוק: ${description}` };
}
async function tool_list_scheduling_rules(_args, _phone) {
    const rows = await prisma.$queryRawUnsafe(`SELECT id, description FROM "SchedulingRule" WHERE active = true ORDER BY "createdAt" DESC LIMIT 30`);
...
function keywordIntent(message) {
    ...
    for (const [cat, re] of INTENT_KEYWORDS)
        if (re.test(m))
            return cat;
    return null;
}
async function classifyIntent(message) {
    ...
    if (!apiKey)
        return 'general';
    const prompt = `סווג את בקשת המשתמש לעסק לקטגוריה אחת בלבד. החזר אך ורק את מילת המפתח באנגלית, בלי שום דבר נוסף.\n` +
    ...
    try {
        ...
        const raw = (data?.candidates?.[0]?.content?.parts || []).map((p) => p.text || '').join('').toLowerCase();
...
async function routedToolDeclarations(role, message) {
    if (role === 'staff' || role === 'guest') {
        return { decls: TOOL_DECLARATIONS.filter((d) => STAFF_TOOLS.includes(d.name)), group: 'self' };
    }
    const cat = await classifyIntent(message);
    const names = new Set([...UNIVERSAL_TOOLS, ...(TOOL_GROUPS[cat] || GENERAL_TOOLS)]);
    return { decls: TOOL_DECLARATIONS.filter((d) => names.has(d.name)), group: cat };
}
export async function runConversationAgent(phone, userMessage) {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey)
        return '⚠️ עוזר ה-AI לא מוגדר (GEMINI_API_KEY חסר).';
...
//# sourceMappingURL=whatsappConversation.js.map