...
export async function driveAccessToken() {
    ...
    if (!access_token)
        throw new Error('failed_to_get_drive_token');
    return access_token;
}
export async function listDriveFiles(folderId, token, mimeTypes) {
    ...
    if (!res.ok)
        throw new Error(`drive_list_${res.status}: ${await res.text()}`);
...
export async function downloadDriveFile(fileId, token) {
    ...
    if (!res.ok)
        throw new Error(`drive_download_${res.status}`);
