...
function sanitizeSchemaForGemini(schema) {
    ...
    for (const [k, v] of Object.entries(schema)) {
        if (k === '$schema' || k === 'additionalProperties' || k === 'patternProperties')
            continue;
        if (k === 'format') {
            if (typeof v === 'string' && allowedFormats.has(v))
                out[k] = v;
            continue;
        }
...
function xmlBodyToText(xml, paraCloseTags) {
    let s = xml;
    for (const tag of paraCloseTags)
        s = s.split(`</${tag}>`).join('\n');
    s = s.replace(/<[^>]+>/g, '');
...
async function xlsxToText(zip) {
    const readIf = async (p) => (zip.file(p) ? await zip.file(p).async('string') : '');
    const ssXml = await readIf('xl/sharedStrings.xml');
    const shared = [];
    for (const m of ssXml.matchAll(/<si>([\s\S]*?)<\/si>/g)) {
        let t = '';
        for (const tm of m[1].matchAll(/<t[^>]*>([\s\S]*?)<\/t>/g))
            t += tm[1];
        ...
        for (const rowM of sx.matchAll(/<row[^>]*>([\s\S]*?)<\/row>/g)) {
            ...
            for (const cM of rowM[1].matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/g)) {
                ...
                if (type === 's') {
                    ...
                    if (vi)
                        val = shared[parseInt(vi[1], 10)] || '';
                }
                else if (type === 'inlineStr' || type === 'str') {
                    const ti = inner.match(/<t[^>]*>([\s\S]*?)<\/t>/);
                    if (ti)
                        val = decodeXmlEntities(ti[1]);
                }
                else {
                    const vi = inner.match(/<v>([\s\S]*?)<\/v>/);
                    if (vi)
                        val = decodeXmlEntities(vi[1]);
                }
                cells.push(val.trim());
            }
...
async function extractDocText(mime, url, buf) {
    try {
        const hint = `${mime} ${url}`.toLowerCase();
        const isZip = buf.length > 4 && buf[0] === 0x50 && buf[1] === 0x4b && (buf[2] === 0x03 || buf[2] === 0x05 || buf[2] === 0x07);
        if (!isZip) {
            if (/\brtf\b|\.rtf/.test(hint) || buf.slice(0, 5).toString('latin1') === '{\\rtf')
                return stripRtf(buf.toString('latin1')).slice(0, 100_000) || null;
            if (/text\/csv|\.csv/.test(hint))
                return buf.toString('utf8').slice(0, 100_000) || null;
            if (/text\/(plain|markdown)|\.txt|\.md\b/.test(hint))
                return buf.toString('utf8').slice(0, 100_000) || null;
        ...
        if (zip.file('word/document.xml')) {
            ...
            for (const s of slides)
                parts.push(xmlBodyToText(await zip.file(s).async('string'), ['a:p']));
            text = parts.join('\n\n');
        }
...
function finalizeFile(mime, url, buf) {
    return extractDocText(mime, url, buf).then((text) => {
        if (text && text.trim())
            return { mime: 'text/plain', data: Buffer.from(text, 'utf8').toString('base64') };
...
// Gemini
// ---------------------------------------------------------------------------
async function geminiInvoke(args) {
...
// ---------------------------------------------------------------------------
async function anthropicInvoke(args) {
    const { prompt, responseSchema, fileUrls, model } = args;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            if (provider === 'anthropic')
                return await anthropicInvoke(args);
...
export async function generateImage({ prompt }) {
    ...
    const recipes = candidates.flatMap((model) => [
        ...
        },
        {
            ...
            extract: (d) => d?.generatedImages?.[0]?.image?.imageBytes || null,
    ...
    for (const r of recipes) {
        ...
        if (res.ok) {
            ...
            if (b64)
                return { image_base64: b64, model: r.tag };
