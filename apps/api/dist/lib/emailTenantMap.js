...
export async function registerEmailTenant(email, slug, restaurantName) {
    const e = norm(email);
    if (!e || !/\S+@\S+\.\S+/.test(e))
        return;
    ...
    try {
        await ensureTable();
        await db.$executeRawUnsafe(`INSERT INTO "platform"."EmailTenantMap" ("email","tenant_slug","restaurant_name","updated_at")
       ...
       DO UPDATE SET "restaurant_name"=COALESCE(EXCLUDED."restaurant_name","platform"."EmailTenantMap"."restaurant_name"),
                     "updated_at"=CURRENT_TIMESTAMP`, e, s, restaurantName || null);
    }
    catch (err) {
        console.warn('[emailTenantMap] register failed', err?.message);
    }
}
// All restaurants an email belongs to (across the fleet).
export async function resolveEmailTenants(email) {
    const e = norm(email);
    if (!e)
        return [];
    try {
        await ensureTable();
        const rows = await db.$queryRawUnsafe(`SELECT tenant_slug, restaurant_name FROM "platform"."EmailTenantMap" WHERE email = $1 ORDER BY updated_at DESC`, e);
        return rows.map(r => ({ slug: String(r.tenant_slug).toLowerCase(), name: r.restaurant_name || null }));
    }
    catch (err) {
        console.warn('[emailTenantMap] resolve failed', err?.message);
        return [];
    }
}
...
export async function backfillEmailTenantMap() {
    const slug = currentTenantSlug();
    let name = null;
    try {
        const p = await db.restaurantProfile.findFirst({ select: { restaurant_name: true } }).catch(() => null);
        name = p?.restaurant_name || null;
        if (!name && slug !== 'alena') {
            const rows = await db.$queryRawUnsafe(`SELECT restaurant_name FROM "Tenant" WHERE slug = $1 LIMIT 1`, slug).catch(() => []);
    ...
    try {
        const emps = await db.employee.findMany({ where: { email: { not: null } }, select: { email: true } });
        emps.forEach(x => { const e = norm(x.email); if (e)
            emails.add(e); });
    }
    catch { /* ignore */ }
    try {
        const users = await db.user.findMany({ where: { email: { not: null } }, select: { email: true } });
        users.forEach(x => { const e = norm(x.email); if (e)
            emails.add(e); });
    }
    catch { /* ignore */ }
    for (const e of emails)
        await registerEmailTenant(e, slug, name);
    return { slug, count: emails.size };
}
//# sourceMappingURL=emailTenantMap.js.map