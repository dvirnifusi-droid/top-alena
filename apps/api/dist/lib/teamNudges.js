...
export async function getNudgeConfig() {
    try {
        const rows = await db.$queryRawUnsafe(`SELECT team_nudges FROM "RestaurantProfile" LIMIT 1`);
...
async function alreadySent(dedupKey) {
    try {
        const rows = await db.$queryRawUnsafe(`SELECT id FROM "WhatsAppMessage" WHERE status='nudge' AND raw->>'dedup_key' = $1 LIMIT 1`, dedupKey);
        return !!rows[0];
    }
    catch {
        return false;
...
async function logNudge(contactPhone, body, dedupKey, kind) {
    try {
        await db.$executeRawUnsafe(`INSERT INTO "WhatsAppMessage" ("id","direction","from_phone","to_phone","contact_phone","body","status","raw","created_at")
       VALUES (gen_random_uuid()::text,'outbound','system',$1,$1,$2,'nudge',$3::jsonb,NOW())`, contactPhone, body, JSON.stringify({ kind, dedup_key: dedupKey }));
    }
    catch (e) {
        console.warn('[nudge] log failed:', e?.message);
    }
}
async function sendNudge(emp, body, dedupKey, kind) {
    if (!emp.phone)
        return false;
    let sent = false;
    ...
    try {
        await notifyEmployee(emp.id, 'תזכורת מההנהלה 📢', body.slice(0, 180));
    }
    catch { /* best-effort */ }
...
async function ownerSummary(text) {
    try {
        ...
        console.warn('[nudge] owner summary failed:', e?.message);
    }
}
async function activeEmployees() {
    try {
        return await db.$queryRawUnsafe(`SELECT id, full_name, phone, department, role FROM "Employee" WHERE status='active'`);
    }
    catch {
        return [];
    }
}
const staffMatches = (emp, a) => (a?.employee_id && String(a.employee_id) === String(emp.id)) ||
...
async function checkAvailability(cfg) {
    ...
    if (await alreadySent(batchKey))
        return;
    await logNudge('system', `batch ${batchKey}`, batchKey, 'availability_batch');
    const [emps, submitted] = await Promise.all([
        activeEmployees(),
        db.$queryRawUnsafe(`SELECT DISTINCT employee_id, employee_name FROM "EmployeeAvailability" WHERE date >= $1::date AND date < $2::date`, startStr, endStr).catch(() => []),
    ]);
    const submittedIds = new Set(submitted.map((s) => String(s.employee_id)));
    const submittedNames = new Set(submitted.map((s) => norm(s.employee_name)));
    const missing = emps.filter((e) => e.phone && !submittedIds.has(String(e.id)) && !submittedNames.has(norm(e.full_name)));
    if (!missing.length) {
        await ownerSummary(`✅ תזכורת זמינות: כל העובדים כבר הגישו לשבוע ${startStr}. אין למי להזכיר.`);
        return;
    }
    let sent = 0;
...
async function checkClockIn(cfg) {
    ...
    const shifts = await db.$queryRawUnsafe(`SELECT id, start_time, shift_type, assigned_staff FROM "WorkShift" WHERE date::date = $1::date`, now.dateStr).catch(() => []);
    if (!shifts.length)
        return;
    const [emps, todayTracking] = await Promise.all([
        activeEmployees(),
        db.$queryRawUnsafe(`SELECT employee_id, employee_name FROM "ShiftTracking" WHERE shift_start::date = $1::date`, now.dateStr).catch(() => []),
    ]);
    const clockedIds = new Set(todayTracking.map((t) => String(t.employee_id)));
    const clockedNames = new Set(todayTracking.map((t) => norm(t.employee_name)));
    const nudged = [];
    for (const ws of shifts) {
        ...
        if (lateBy < cfg.clockin.delay_min || lateBy > cfg.clockin.delay_min + 90)
            continue;
        ...
        for (const a of staff) {
            ...
            if (!emp?.phone)
                continue;
            if (clockedIds.has(String(emp.id)) || clockedNames.has(norm(emp.full_name)))
                continue;
    ...
    if (nudged.length)
        await ownerSummary(`⏰ נשלחו תזכורות החתמת כניסה:\n${nudged.map((n) => `• ${n}`).join('\n')}`);
}
...
async function checkChecklists(cfg) {
    ...
    const now = ilParts();
    let slot = null;
    if (now.hour === cfg.checklist.morning_hour)
        slot = 'morning';
    else if (now.hour === cfg.checklist.evening_hour)
        slot = 'evening';
    if (!slot)
        return;
    ...
    const checklists = await db.$queryRawUnsafe(`SELECT id, title, shift, items FROM "Checklist" WHERE status='active'`).catch(() => []);
    ...
    for (const cl of relevant) {
        ...
        if (!total)
            continue;
        const runs = await db.$queryRawUnsafe(`SELECT results FROM "ChecklistExecution"
       WHERE checklist_id = $1 AND id LIKE 'live\\_%'
         AND COALESCE(status,'in_progress') NOT IN ('completed','requires_attention')
       ORDER BY "createdAt" DESC LIMIT 1`, cl.id).catch(() => []);
        // completed today already? then it's done — skip
        const doneToday = await db.$queryRawUnsafe(`SELECT id FROM "ChecklistExecution"
       WHERE checklist_id = $1 AND status IN ('completed','requires_attention') AND "updatedAt"::date = $2::date LIMIT 1`, cl.id, now.dateStr).catch(() => []);
        if (doneToday[0])
            continue;
        const res = runs[0]?.results && typeof runs[0].results === 'object' && !Array.isArray(runs[0].results) ? runs[0].results : {};
        const done = Object.values(res).filter((v) => v?.checked).length;
        if (done < total)
    ...
    const onShift = await db.$queryRawUnsafe(`SELECT employee_id, employee_name FROM "ShiftTracking"
     WHERE shift_start::date = $1::date AND shift_end IS NULL`, now.dateStr).catch(() => []);
    ...
    for (const t of onShift) {
        ...
        if (emp?.phone && !recipients.some((r) => r.id === emp.id))
            recipients.push(emp);
    }
    ...
    for (const emp of recipients.slice(0, 8)) {
        ...
        if (await sendNudge(emp, body, `${batchKey}_${emp.id}`, 'checklist'))
            sent++;
    }
...
export async function resolveBroadcastRecipients(audience, department) {
    ...
    if (audience === 'tonight' || audience === 'today') {
        const now = ilParts();
        const shifts = await db.$queryRawUnsafe(`SELECT assigned_staff FROM "WorkShift" WHERE date::date = $1::date`, now.dateStr).catch(() => []);
        const out = [];
        for (const ws of shifts) {
            const staff = Array.isArray(ws.assigned_staff) ? ws.assigned_staff : [];
            for (const a of staff) {
                const emp = emps.find((e) => staffMatches(e, a));
                if (emp && !out.some((o) => o.id === emp.id))
                    out.push(emp);
            }
        }
        return out;
...
export async function sendTeamBroadcast(message, audience, department) {
    const recipients = await resolveBroadcastRecipients(audience, department);
    let sent = 0;
    const stamp = Date.now();
    for (const emp of recipients) {
        const body = `📢 הודעה מההנהלה:\n${message}`;
        if (await sendNudge(emp, body, `broadcast_${stamp}_${emp.id}`, 'broadcast'))
            sent++;
    }
...
export async function checkTeamNudges() {
    ...
    try {
        ...
        console.error('[nudge] availability:', e?.message);
    }
    try {
        await checkClockIn(cfg);
    }
    catch (e) {
        console.error('[nudge] clockin:', e?.message);
    }
    try {
        await checkChecklists(cfg);
    }
    catch (e) {
        console.error('[nudge] checklist:', e?.message);
    }
}
//# sourceMappingURL=teamNudges.js.map