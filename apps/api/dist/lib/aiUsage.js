...
const FALLBACK_PRICING = { in_usd_per_m: 0.10, out_usd_per_m: 0.40 };
function computeCostIls(model, tokensIn, tokensOut) {
...
async function ensureAiUsageTable() {
  ...
  `);
    await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "AiUsage_tenant_day_idx" ON "platform"."AiUsage"("tenant_slug", "day")`);
 ...
 */
export async function writeAiUsage(w) {
    try {
        ...
        await prisma.$executeRawUnsafe(`INSERT INTO "platform"."AiUsage" ("tenant_slug","fn_name","model","tokens_in","tokens_out","cost_ils","day")
       VALUES ($1, $2, $3, $4, $5, $6, CURRENT_DATE)`, tenant, fn, model, tokensIn, tokensOut, cost);
    }
    catch (e) {
 ...
 */
export async function getMyMonthlyUsage(tenantSlug) {
    try {
        await ensureAiUsageTable();
        const tenant = (tenantSlug || currentTenantSlug()).toLowerCase();
        const byDay = await prisma.$queryRawUnsafe(`SELECT day::text AS day,
       ...
       GROUP BY day ORDER BY day ASC`, tenant);
        const byFn = await prisma.$queryRawUnsafe(`SELECT fn_name,
              COALESCE(SUM(tokens_in + tokens_out), 0)::int AS tokens,
              COALESCE(SUM(cost_ils), 0)::float AS cost_ils,
              COUNT(*)::int AS calls
       FROM "platform"."AiUsage"
       WHERE tenant_slug = $1 AND day >= date_trunc('month', CURRENT_DATE)
       GROUP BY fn_name ORDER BY cost_ils DESC`, tenant);
