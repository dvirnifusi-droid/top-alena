...
function sanitize(modelName, raw) {
...
export const importRoutes = async (app) => {
        ...
        for (const [modelName, rows] of Object.entries(data)) {
            ...
            for (const raw of rows) {
                ...
                try {
                    ...
                    failed++;
                    if (failed <= 3)
                        req.log.warn({ model: modelName, msg: err.message }, 'import row failed');
                }
            }
    ...
    app.post('/entity/:name', async (req, reply) => {
        ...
        if (!delegate)
            return reply.code(404).send({ error: 'unknown_entity', name });
        if (!Array.isArray(rows))
            return reply.code(400).send({ error: 'rows_must_be_array' });
        let inserted = 0;
        let failed = 0;
        const errors = [];
        for (const raw of rows) {
            ...
            try {
                ...
                failed++;
                if (errors.length < 3)
                    errors.push(err.message?.slice(0, 200) ?? 'unknown');
            }
        }
