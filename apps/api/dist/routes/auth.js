...
export const authRoutes = async (app) => {
    ...
    async function issueSessionForEmail(reply, email, name) {
        ...
        if (!user) {
            ...
            if (!employee)
                return null; // not_registered on this tenant
    ...
    app.post('/google', async (req, reply) => {
        ...
        if (!credential)
            return reply.code(400).send({ error: 'missing_credential' });
        if (!GOOGLE_CLIENT_ID)
            return reply.code(500).send({ error: 'google_not_configured' });
        let payload;
        try {
        ...
        if (tenants.length > 0) {
            const handoff = await reply.jwtSign({ email, name: payload?.name ?? null, purpose: 'google_handoff' }, { expiresIn: '5m' });
            const list = tenants.map(t => ({ slug: t.slug, name: t.name, url: tenantUrl(t.slug) }));
            if (list.length === 1)
                return { route_to: list[0], handoff };
    ...
    app.post('/google-handoff', async (req, reply) => {
        ...
        const handoff = await reply.jwtSign({ email, name: payload?.name ?? null, purpose: 'google_handoff' }, { expiresIn: '2m' });
        return { handoff };
    });
    ...
    app.post('/google-consume', async (req, reply) => {
        ...
        if (!handoff)
            return reply.code(400).send({ error: 'missing_handoff' });
        let decoded;
        try {
