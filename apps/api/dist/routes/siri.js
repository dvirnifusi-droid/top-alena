...
async function executeQuery(cmd) {
    ...
    switch (cmd.intent) {
        case 'q_today_summary': {
            ...
            const valid = resv.filter(r => !['cancelled', 'no_show'].includes(r.status || ''));
            const guests = valid.reduce((s, r) => s + (Number(r.party_size) || 0), 0);
            const inQ = queue.filter(q => q.status === 'pending' || q.status === 'active').length;
            const staff = shifts.flatMap(s => s.assigned_staff || []).length;
            const todayTip = tips.find(t => String(t.date).slice(0, 10) === today);
        ...
        case 'q_status_summary': {
            ...
            const valid = resv.filter(r => !['cancelled', 'no_show'].includes(r.status || ''));
            const seated = resv.filter(r => r.status === 'seated').length;
            const inQ = queue.filter(q => q.status === 'pending' || q.status === 'active').length;
        ...
        case 'q_next_reservation': {
            ...
            const upcoming = resv.filter(r => !['cancelled', 'no_show', 'seated', 'completed'].includes(r.status || 'pending') && r.time);
            if (upcoming.length === 0)
                return 'אין יותר הזמנות היום';
            const r = upcoming[0];
        ...
        case 'q_queue_count': {
            ...
            const guests = queue.reduce((s, q) => s + (Number(q.party_size) || 0), 0);
        ...
        case 'q_free_tables': {
            ...
            const tables = layout?.[0]?.tables || [];
            const occupied = new Set(sessions.flatMap(s => String(s.table_number || '').split(/[,+]/).map(p => p.trim())));
        ...
        case 'q_today_revenue': {
            const resv = await prisma.reservation.findMany({ where: { date: today } }).catch(() => []);
            const seated = resv.filter(r => ['seated', 'completed'].includes(r.status));
        ...
        case 'q_tips_per_hour': {
            ...
            const rep = reports.find(r => String(r.date).slice(0, 10) === today);
            if (!rep)
                return 'אין דוח טיפים היום';
            const total = Math.round(Number(rep.total_tips_collected) || 0);
            const totalHours = (rep.staff_details || []).reduce((s, x) => s + (Number(x.hours) || 0), 0);
        ...
        case 'q_low_stock': {
            ...
            const low = items.filter(i => i.min_threshold && Number(i.current_stock) <= Number(i.min_threshold));
            if (low.length === 0)
                return 'מלאי תקין';
        ...
        case 'q_inventory_value': {
            const items = await prisma.inventory.findMany().catch(() => []);
            const total = items.reduce((s, i) => s + (Number(i.current_stock || 0) * Number(i.cost_per_unit || 0)), 0);
        ...
        case 'q_pending_deliveries': {
            ...
            const pending = ds.filter(d => d.delivery_status === 'pending' || !d.courier_name);
            if (pending.length === 0)
                return 'אין משלוחים ממתינים';
        ...
        case 'q_clocked_in_now': {
            ...
            if (tracks.length === 0)
                return 'אין עובדים פעילים';
            const names = tracks.map(t => t.employee_name).join(', ');
            return `${tracks.length} עובדים פעילים: ${names}`;
        }
        case 'q_compare_revenue': {
            const today = new Date();
            const get = async (offsetDays) => {
                const dates = [];
                for (let i = 0; i < 7; i++) {
                ...
                const all = await Promise.all(dates.map(d => prisma.reservation.findMany({ where: { date: d } }).catch(() => [])));
                return all.flat().filter(r => ['seated', 'completed'].includes(r.status))
        ...
        case 'q_on_shift_lunch': {
            ...
            let filtered = shifts;
            if (cmd.intent === 'q_on_shift_evening')
                filtered = filtered.filter(s => s.shift_type === 'dinner');
            if (cmd.intent === 'q_on_shift_lunch')
                filtered = filtered.filter(s => s.shift_type === 'lunch');
        ...
        case 'q_events_week': {
            ...
            const upcoming = bookings.filter(b => b.event_date >= today && b.event_date <= weekEndStr && b.status !== 'cancelled');
            if (upcoming.length === 0)
                return 'אין אירועים השבוע';
            return `${upcoming.length} אירועים השבוע`;
        }
        case 'q_birthdays_today': {
            const resv = await prisma.reservation.findMany({ where: { date: today } }).catch(() => []);
            const todays = resv.filter(r => String(r.special_occasion || '').match(/יום הולדת|birthday/i) &&
                !['cancelled', 'no_show'].includes(r.status || 'pending'));
            if (todays.length === 0)
                return 'אין חוגגי יום הולדת היום';
        ...
        case 'q_late_today': {
            ...
            for (const ws of shifts) {
                for (const a of (ws.assigned_staff || [])) {
                    const t = tracks.find(tr => tr.employee_id === a.employee_id &&
...
export const siriRoutes = async (app) => {
    ...
    app.post('/command-text', async (req, reply) => {
        const sentKey = (req.headers['x-api-key'] || req.headers['x-siri-key'] || '');
        ...
        try {
            ...
            const parsed = await handler({ body: { text }, user: fakeUser, query: {} });
            reply.type('text/plain; charset=utf-8');
    ...
    app.post('/command', async (req, reply) => {
        ...
        const text = String(body.text || '').trim();
        if (!text)
            return reply.code(400).send({ ok: false, spoken: 'לא קיבלתי טקסט' });
        ...
        try {
            const handler = functionHandlers['parseVoiceCommand'];
            if (!handler)
                return reply.code(500).send({ ok: false, spoken: 'מערכת הפענוח לא זמינה' });
