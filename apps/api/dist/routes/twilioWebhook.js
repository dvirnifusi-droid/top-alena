...
import { resolveTenantFromMessage, setLastTenant, currentTenantSlug, containerUrlForSlug, } from '../lib/whatsappRouter.js';
const STRICT_SIG = false;
function verifyTwilioSignature(authToken, fullUrl, params, signature) {
...
function stripWhatsAppPrefix(s) {
    if (!s)
        return '';
    return String(s).replace(/^whatsapp:/i, '').trim();
}
export const twilioWebhookRoutes = async (app) => {
        ...
        try {
            if (isStatusCallback) {
                ...
                if (numMedia >= 1 && params.MediaUrl0 && String(params.MediaContentType0 || '').toLowerCase().startsWith('audio/')) {
                    try {
                        ...
                        if (!isKnownBusinessMember && await isOnboardingActive(from)) {
                            reply.type('text/xml').send('<?xml version="1.0" encoding="UTF-8"?><Response><Message>🎙️ קיבלתי את ההקלטה, רגע מתמלל...</Message></Response>');
                            void (async () => {
                                try {
                                    const transcript = await transcribeWhatsAppVoice(params.MediaUrl0);
                                    if (transcript)
                                        await tryHandleOnboardingMessage(from, transcript);
                                    else
                                        await sendWhatsApp(from, 'לא הצלחתי לתמלל את ההקלטה 🤔 אפשר לכתוב לי במקום?');
                                }
                                catch (e) {
                ...
                if (numMedia >= 1 && params.MediaUrl0) {
                    try {
                        const claimed = await tryHandleOnboardingMedia(from, params.MediaUrl0, String(params.MediaContentType0 || ''));
                        if (claimed) {
                            reply.type('text/xml').send('<?xml version="1.0" encoding="UTF-8"?><Response><Message>📥 קיבלתי את הקובץ! קורא ומטמיע במערכת... תוך כדקה אשלח סיכום 🚀</Message></Response>');
                            return;
                        }
                    }
                    catch (e) {
                ...
                if (isWhatsAppAdmin(from)) {
                    ...
                    if (numMedia >= 1 && params.MediaUrl0) {
                        ...
                        await prisma.whatsAppMessage.create({
                            data: {
                                ...
                                raw: { ...params, admin_invoice: !isAudio, admin_voice: isAudio },
                                is_read: true,
                            },
                        }).catch(() => { });
                        reply.type('text/xml').send(isAudio
                        ...
                        void (async () => {
                            try {
                                if (isAudio) {
                                    ...
                                    if (actionConfirm) {
                                        await sendWhatsApp(from, actionConfirm);
                                        return;
                                    }
                                    const invConfirm = await tryConfirmPendingInvoice(from, transcript);
                                    if (invConfirm) {
                                        await sendWhatsApp(from, invConfirm);
                                        return;
                                    }
                    ...
                    if (scanConfirmReply) {
                        await prisma.whatsAppMessage.create({
                            data: {
                                ...
                                raw: { ...params, admin_scan_confirm: true }, is_read: true,
                            },
                        }).catch(() => { });
                        ...
                        const escapedScan = scanConfirmReply
                            ...
                            .replace(/"/g, '&quot;').replace(/'/g, '&apos;');
                        reply.type('text/xml').send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${escapedScan}</Message></Response>`);
                        return;
                    }
                    ...
                    if (actionConfirmReply) {
                        await prisma.whatsAppMessage.create({
                            data: {
                                twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                                contact_phone: from, body, num_media: numMedia, status: 'received',
                                raw: { ...params, admin_action_confirm: true }, is_read: true,
                            },
                        }).catch(() => { });
                        ...
                        const escapedAct = actionConfirmReply
                            .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
                            .replace(/"/g, '&quot;').replace(/'/g, '&apos;');
                        reply.type('text/xml').send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${escapedAct}</Message></Response>`);
                        return;
                    }
                    ...
                    if (confirmReply) {
                        await prisma.whatsAppMessage.create({
                            data: {
                                twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                                contact_phone: from, body, num_media: numMedia, status: 'received',
                                raw: { ...params, admin_invoice_confirm: true }, is_read: true,
                            },
                        }).catch(() => { });
                        ...
                        const escapedConfirm = confirmReply
                            .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
                            .replace(/"/g, '&quot;').replace(/'/g, '&apos;');
                        reply.type('text/xml').send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${escapedConfirm}</Message></Response>`);
                        return;
                    }
                    ...
                    if (hoursReply) {
                        await prisma.whatsAppMessage.create({
                            data: {
                                twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                                contact_phone: from, body, num_media: numMedia, status: 'received',
                                raw: { ...params, admin_hours_reply: true }, is_read: true,
                            },
                        }).catch(() => { });
                        ...
                        const escapedHours = hoursReply
                            .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
                            .replace(/"/g, '&quot;').replace(/'/g, '&apos;');
                        reply.type('text/xml').send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${escapedHours}</Message></Response>`);
                        return;
                    }
                    ...
                    if (isBuildCmd || isReplaceCmd) {
                        await prisma.whatsAppMessage.create({
                            data: {
                                twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                                contact_phone: from, body, num_media: numMedia, status: 'received',
                                raw: { ...params, admin_schedule_cmd: true }, is_read: true,
                            },
                        }).catch(() => { });
                        reply.type('text/xml').send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${isReplaceCmd ? '🔄 מחליף... כ-5 שניות' : '⏳ בונה סידור... כ-30 שניות'}</Message></Response>`);
                        void (async () => {
                            try {
                                ...
                                try {
                                    await sendWhatsApp(from, `⚠️ שגיאה בבניית סידור: ${e?.message || 'unknown'}`);
                                }
                                catch { /* noop */ }
                            }
                        })();
                    ...
                    if (agentReply && agentReply !== '🤔 לא הבנתי את הפקודה. שלח/י *עזרה* לרשימת פקודות.') {
                        ...
                        await prisma.whatsAppMessage.create({
                            data: {
                                twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                                contact_phone: from, body, num_media: numMedia, status: 'received',
                                raw: { ...params, admin_command: true }, is_read: true,
                            },
                        }).catch(() => { });
                        ...
                        const escaped = agentReply
                            .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
                            .replace(/"/g, '&quot;').replace(/'/g, '&apos;');
                        reply.type('text/xml').send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>${escaped}</Message></Response>`);
                        return;
                    }
                    ...
                    await prisma.whatsAppMessage.create({
                        data: {
                            twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                            contact_phone: from, body, num_media: numMedia, status: 'received',
                            raw: { ...params, admin_action_attempt: true }, is_read: true,
                        },
                    }).catch(() => { });
                    reply.type('text/xml').send('<?xml version="1.0" encoding="UTF-8"?><Response></Response>');
                    void (async () => {
                        try {
                            ...
                            try {
                                await sendWhatsApp(from, `⚠️ שגיאה: ${e?.message || 'unknown'}`);
                            }
                            catch { /* noop */ }
                        }
                    })();
                    ...
                    if (scope.role !== 'guest') {
                        ...
                        await prisma.whatsAppMessage.create({
                            data: {
                                twilio_sid: sid || null, direction: 'inbound', from_phone: from, to_phone: to,
                                contact_phone: from, body, num_media: numMedia, status: 'received',
                                raw: { ...params, agent_role: scope.role, employee_id: scope.employee_id }, is_read: true,
                            },
                        }).catch(() => { });
                        reply.type('text/xml').send('<?xml version="1.0" encoding="UTF-8"?><Response></Response>');
                        void (async () => {
                            try {
                                ...
                                try {
                                    await sendWhatsApp(from, '⚠️ סליחה, היה לי באג. נסה שוב בעוד דקה.');
                                }
                                catch { /* noop */ }
                            }
                        })();
                ...
                if (isUnsubscribe) {
                    ...
                    reply.type('text/xml').send('<?xml version="1.0" encoding="UTF-8"?><Response><Message>הוסרת מרשימת הדיוור של עלינא ✅ לא תקבל/י מאיתנו עוד הודעות שיווק. אם תרצה/י לחזור — אפשר להירשם שוב בכל ביקור 🌿</Message></Response>');
                    return;
                }
                ...
                }).catch((e) => { req.log.error({ err: e?.message }, '[twilio-webhook] insert failed'); });
                // Push notification to admin so they see new message even if not in app
                try {
                    await pushoverToAdmins(`💬 ${from}`, body.slice(0, 200));
                }
                catch { /* best effort */ }
            }
        }
    ...
    app.post('/campaign-status', async (req, reply) => {
        ...
        try {
            ...
            if (sid && !recipient.twilio_sid)
                updates.twilio_sid = sid;
