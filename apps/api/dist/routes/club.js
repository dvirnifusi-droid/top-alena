...
export async function clubRoutes(app) {
    ...
    app.post('/register', async (req, reply) => {
        ...
        if (!parsed.success)
            return reply.code(400).send({ error: 'bad_body', detail: parsed.error.flatten() });
        const phone = normalizePhone(parsed.data.phone);
        ...
        const tagsExtra = {};
        if (parsed.data.anniversary)
            tagsExtra.anniversary = parsed.data.anniversary;
    ...
    app.post('/orders', async (req, reply) => {
        ...
        const punch = await maybeGrantPunchCard(customer.id, newVisitCount).catch(() => null);
    ...
    app.post('/redeem', async (req, reply) => {
        ...
        req.log.info({ phone, order_id: parsed.data.order_id, coins, newBalance, ilsDiscount }, 'club_coins_redeemed');
        return {
    ...
    app.get('/benefits/:phone', async (req, reply) => {
