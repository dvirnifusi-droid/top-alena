...
// `hidden` tabs stay defined but are not shown — the strategy hub now covers
// the agents/inbox/prompts flows, so they're hidden for now (owner, 2026-07-27).
// To bring one back, just remove its `hidden: true`.
const TABS = [
    { id: 'inbox', label: 'תיבת הסוכן', icon: Inbox, C: AgentInbox, page: 'AgentInbox', hidden: true },
    ...
    { id: 'agents', label: 'סוכני שיווק', icon: Megaphone, C: MarketingAgentsHub, page: 'MarketingAgentsHub', hidden: true },
    { id: 'prompts', label: 'פרומפטים', icon: Brain, C: AgentPrompts, page: 'AgentPrompts', hidden: true },
...
export default function AIHub() {
    const [tab, setTab] = useState(() => {
        ...
        return VISIBLE.find(t => t.id === h) ? h : VISIBLE[0].id;
    });
        ...
        <div className="p-4" dir="rtl">
            ...
            <Tabs value={tab} onValueChange={onChange}>
                <div className="sticky top-0 z-10 bg-white -mx-4 px-4 pb-2 mb-3 border-b">
                    <TabsList
                        className="flex w-full overflow-x-auto h-auto p-1 gap-1 justify-start md:grid"
