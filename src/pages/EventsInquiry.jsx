  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-100 flex flex-col" dir="rtl">
      ...
      <main className="flex-1 max-w-2xl w-full mx-auto p-4 flex flex-col">
        <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 pb-4">
              ...
              <div
                className={`max-w-[85%] rounded-2xl px-4 py-2.5 shadow-sm whitespace-pre-wrap leading-relaxed ${
                  m.role === 'user'
                    ...
                    : 'bg-white text-slate-800 rounded-bl-sm border border-emerald-100'
                }`}
              >
                {m.content}
              </div>
          ...
          {sending && (
            <div className="flex justify-end">
              <div className="bg-white text-slate-500 rounded-2xl rounded-bl-sm px-4 py-2 shadow-sm text-sm border border-emerald-100">
                כותבת…
              </div>
            </div>
          )}
        ...
        {/* Error */}
        {error && (
          <div className="rounded-xl p-3 my-2 bg-red-50 border border-red-200 text-red-800 text-sm">{error}</div>
        )}
        ...
        {!done && (
          <div className="bg-white border border-emerald-200 rounded-2xl p-2 flex items-end gap-2 shadow-sm">
            <textarea
              ...
              onKeyDown={onKey}
              placeholder="הקלידו את התשובה ולחצו שלח…"
              disabled={sending}
              rows={1}
